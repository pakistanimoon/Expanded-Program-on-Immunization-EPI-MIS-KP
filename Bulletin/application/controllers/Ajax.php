...
class Ajax extends CI_Controller {
	...
	public function completenessTimelinessColumn2D(){
		$result['fweekfrom'] = $fweekfrom = $this -> input -> get('fweekfrom');
		$result['fweekto'] = $fweekto = $this -> input -> get('fweekto');
		$result = districtWiseCommulativeTimelinessAndCompletenessForaWeek($fweekfrom,$fweekto);
	...
	public function ageWiseDistributionOfLabConfirmedCases(){
		$result['fweekfrom'] = $fweekfrom = $this -> input -> get('fweekfrom');
		$result['fweekto'] = $fweekto = $this -> input -> get('fweekto');
		$result = $this -> bulletin -> ageWiseDistributionOfLabConfirmedCases($fweekfrom,$fweekto);
	...
	public function measlesSuspectedCasesWithLabResultsAgeWiseCount(){
		...
		$result = $this -> bulletin -> ageWiseDistributionOfSuspectedAndLabResultCases($fweekfrom,$fweekto);
	...
	public function measlesCasesReceivedDoseWiseCount(){
		$result['fweekfrom'] = $fweekfrom = $this -> input -> get('fweekfrom');
		$result['fweekto'] = $fweekto = $this -> input -> get('fweekto');
		$result = $this -> bulletin -> measlesCasesReceivedDoseWiseCount($fweekfrom,$fweekto);
		foreach($result as $key => $value){
