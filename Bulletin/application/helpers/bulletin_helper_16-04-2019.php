...
if(!function_exists('noOfFacilitiesWithZeroReportSubmittedInaWeek')){
	function noOfFacilitiesWithZeroReportSubmittedInaWeek($year,$fweekfrom,$fweekto){
		...
		//$query = "SELECT COUNT(DISTINCT facode) AS cnt FROM zero_report WHERE fweek >= '{$fweekfrom}' AND fweek <= '{$fweekto}' AND report_submitted='1'";
		$result = $CI -> db -> query($query) -> row();
		return $result -> cnt;
	}
}
...
if (!function_exists('percentageOfDistrictsAchived100PercComplianceInaWeek')){
	function percentageOfDistrictsAchived100PercComplianceInaWeek($fweekfrom,$fweekto){
		$CI = & get_instance();
		$query = "
	...
	function countOfVPDs($fweekfrom,$fweekto){
		...
		$query = "
					SELECT 
						...
						(SELECT COUNT(*) FROM case_investigation_db WHERE fweek >= '{$fweekfrom}' AND fweek <= '{$fweekto}' AND distcode=districts.distcode and fweek NOT LIKE  '%-00') as measles, 
