...
class CompliancesTest extends CI_Controller {
	//================ Constructor Function Starts ==================//
	public function __construct() {
		parent::__construct();
		$this -> load -> model('Compliances_model_test');
	...
	public function compliancesFilters(){
		...
		// $reportPeriod = array('cryearly');
			...
			$reportPeriod = array('cryearly','from_week','to_week');
		else if ($functionName == 'HFMVRFTest' || $functionName == 'HF-Consumption-Requisition-Test')
			//$reportPeriod = array('compliance_year','monthfrom','monthto');
			$reportPeriod = array('month-from-to-previous');
		else
			$reportPeriod = NULL;
		$functionName = str_replace("-", "_", $functionName);
		$reportPath = base_url()."CompliancesTest/".$functionName;
	...
	function reportTitle($functionName){
		...
		switch($functionName){
			case "HFMVRFTest":
				...
				break;
			case "HF_Consumption_Requisition_Test":
	...
	function HFMVRFTest($distcode=NULL, $monthfrom=NULL, $monthto=NULL){
		//echo "danish";exit;
		//print_r($_POST); exit();
		...
		$dataHFMVRF['fileToLoad'] = 'compliances/hfmvrf_test';		
	...
	function HF_Consumption_Requisition_Test($distcode=NULL, $monthfrom=NULL, $monthto=NULL){
		...
		$dataHFCRC['data'] = $this -> Compliances_model_test -> HF_Consumption_Requisition_Test($data,$dataHFCRC['pageTitle']);
		$dataHFCRC['fileToLoad'] = 'compliances/hf_consumption_requisition_test';
