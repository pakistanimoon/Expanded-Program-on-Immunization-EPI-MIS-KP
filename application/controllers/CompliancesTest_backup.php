...
class CompliancesTest extends CI_Controller {
	...
	public function compliancesFilters(){
		...
		if($functionName == 'HF-Consumption-Requisition' || $functionName == 'AEFI-Compliance')
		...
		// $reportPeriod = array('cryearly');
			...
			$reportPeriod = array('cryearly','from_week','to_week');
		else if ($functionName == 'HFMVRFTest')
	...
	function HFMVRFTest($distcode=NULL, $year=NULL, $monthfrom=NULL, $monthto=NULL){
		//echo "danish";exit;
		//print_r($_POST); exit();
		if($distcode && $year){
			$data = array("distcode"=>$distcode, "year"=>$year, "monthfrom"=>$monthfrom, "monthto"=>$monthto);
		}else{			
