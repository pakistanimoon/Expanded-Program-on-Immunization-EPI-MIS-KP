<?php
class Measles_investigation extends CI_Controller {
	...
	public function __construct() {
		...
		$this -> load -> model ('Measles_investigation_model');
	...
	//-------------------------------------------------------------------------------------------------//
	public function getSpecimenList(){
		$id = $this -> input -> post('id');
		//$data['LinkedCase'] = $this -> Ajax_cross_notified_model -> getLinked_CaseInformation($linked_epid_number);
		$query = "SELECT type_specimen from case_investigation_db where case_type='Msl' and id = $id ";
		$result = $this-> db-> query($query);
		$data['specimenList'] = $result-> row_array();
		//$specimenList = explode(',', $specimenList);
		//$data['specimenList'] = $specimenList;
		//print_r($data);
		echo json_encode($data['specimenList']);
	} 
	public function measles_investigation_list(){
		...
		$query = "SELECT max(id) as max_id from case_investigation_db where $wcd";
		//echo $query; exit();
	...
	public function measles_investigation(){
		$data['data']=$this -> Measles_investigation_model -> measles_investigation(); 
		...
		$data['fileToLoad'] = 'investigation_forms/measles_investigation_form';
		$data['pageTitle']='Measles Investigation Form | EPI-MIS';
		$this->load->view('template/epi_template',$data);	
	}

	public function measles_investigation_save(){
		//print_r($_POST);exit();
		...
		if(($this -> input -> post('facode') || $this -> input -> post('cross_notified')) && $this -> input -> post('year'))
		{
		   ...
		   $specimen_collected = $this-> input-> post('specimen_collected');
		   $lab_final_classification = $this-> input-> post('lab_final_classification');
		   ...
		   //----------------------- Epidemiology Linked Fields -----------------------//
		   	$epidem_linked_case = ($this-> input-> post('epidem_linked_case'))?$this-> input-> post('epidem_linked_case'):NULL;
			$linked_epid_number = ($this-> input-> post('linked_epid_number'))?$this-> input-> post('linked_epid_number'):'';
			$final_classification = ($this-> input-> post('final_classification'))?$this-> input-> post('final_classification'):'';
			$specimen_quantity_adequate = ($this-> input-> post('specimen_quantity_adequate'))?$this-> input-> post('specimen_quantity_adequate'):NULL;
			$clinically_compatible_with = ($this-> input-> post('clinically_compatible_with'))?$this-> input-> post('clinically_compatible_with'):'';
			//--------------------------------- Array ---------------------------------//
			/////////////////////////////////////////////////////////////////////////////	
		    ...
		    $newlist=array();
			if(isset($symptomlist) && ! empty($symptomlist) && is_array($symptomlist)){
			...
			$specimenlist = ($this-> input-> post('type_specimen'))?$this-> input-> post('type_specimen'):'';
		    $newlist=array();
			if(isset($specimenlist) && ! empty($specimenlist) && is_array($specimenlist)){
				foreach($specimenlist as $row){
					$newlist[] = $row;
				}
				$specimen=implode(',',$newlist);
			}else{
		   ...
		   $DataArray=array(
			   ...
			   'lab_final_classification' => $lab_final_classification,
			   ...
			   'specimen_received_date' => (isset($specimen_received_date) AND $specimen_received_date != '')?$specimen_received_date:NULL,	
				...
				//--- Epidemiology Linked Fields ---//
				'epidem_linked_case' => $epidem_linked_case,
				'linked_epid_number' => $linked_epid_number,
				'final_classification' => $final_classification,
				'specimen_quantity_adequate' => $specimen_quantity_adequate,
				'clinically_compatible_with' => $clinically_compatible_with
			);			
			...
			{	
				...
				if(($procode != $_SESSION["Province"]) || ($approval_status == 'Approved')){
					...
					$filepath = 'Measles_investigation/measles_investigation_receive_and_save';
	...
	public function measlesInvestigation_Approve(){
		...
		if($this -> input -> post('facode')>0 && $this -> input -> post('case_epi_no')){
			...
			if($procode != $_SESSION["Province"]){
				$filepath = 'Measles_investigation/measlesApprove_and_save'; 
				$url = $this -> getSingleRegionUrl($procode); 
	...
	public function measlesApprove_and_save(){
		//dataEntryValidator(0);
		if($this -> input -> post('facode')>0 && $this -> input -> post('case_epi_no')){
			$distcode = $this -> session -> District;
			$query = "SELECT epid_code from districts where distcode='$distcode'";
	...
	public function measles_investigation_edit(){
		...
		//print_r($data['measles_Result']);exit();
		...
		if($data != 0){			
			...
			$data1['fileToLoad'] = 'investigation_forms/measles_investigation_form_edit';
			$data1['pageTitle']='Measles Investigation Form | EPI-MIS';
	...
	public function measles_investigation_view(){
		$id = $this -> uri -> segment(3);
		$year = $this -> uri -> segment(4);
		$pcode = "SELECT procode from case_investigation_db where id = $id";
		...
		if($data != 0){
			$data1['data']=$data;
			//print_r($data1['data']);exit();
			$data1['fileToLoad'] = 'investigation_forms/measles_investigation_form_view';
			$data1['pageTitle']='Measles Investigation Form | EPI-MIS';
			$this->load->view('template/epi_template',$data1);
		}
		else{
	...
	function measles_investigation_receive_and_save(){
		...
		if($this -> input -> post('cross_notified') && $this -> input -> post('year'))
		{
		   ...
		   $submitted_date = ($this-> input-> post('submitted_date'))?date('Y-m-d', strtotime($this-> input-> post('submitted_date'))):NULL;
			...
			$final_classification = ($this-> input-> post('final_classification'))?$this-> input-> post('final_classification'):'';
			$specimen_quantity_adequate = ($this-> input-> post('specimen_quantity_adequate'))?$this-> input-> post('specimen_quantity_adequate'):NULL;
			$clinically_compatible_with = ($this-> input-> post('clinically_compatible_with'))?$this-> input-> post('clinically_compatible_with'):NULL;
			//--------------------------------- Array ---------------------------------//
			/////////////////////////////////////////////////////////////////////////////	
		   //$clinical_representation = $this-> input-> post('clinical_representation');		   
		   $symptomlist = ($this-> input-> post('clinical_representation'))?$this-> input-> post('clinical_representation'):0;
