	...
	public function driverdb_save() {
		...
		//echo '<pre>';print_r($this->input->post());echo '</pre>';exit;
			...
			$driverData = array(
				...
				'phone' => ($this -> input -> post ('phone'))? $this -> input -> post ('phone') : Null,
				'branchname' => ($this -> input -> post ('branchname'))? $this -> input -> post ('branchname') : Null,
				'basicpayscale' => ($this -> input -> post ('basicpayscale'))? $this -> input -> post ('basicpayscale') : Null,
				'bankaccount' => ($this -> input -> post ('bankaccount'))? $this -> input -> post ('bankaccount') : Null,
				'bid' => ($this -> input -> post ('bankid'))? $this -> input -> post ('bankid') : Null,
				'is_temp_saved' => $this -> input -> post ('is_temp_saved'),
				
				'totalmonthlysalary' => ($this -> input -> post ('totalmonthlysalary'))? $this -> input -> post ('totalmonthlysalary') : Null
							
			);
			$data = $this -> System_setup_model -> driverdb_save($driverData,$drivercode);
		}else{
...
public function codb_list() {
	...
	public function mfpdb_save() {
		...
		$this->load->library('form_validation');
		$this->form_validation->set_error_delimiters('<div class="error" style="color:red;">', '</div>');
		$this->form_validation->set_rules('mfpname','Measles Focal Person Name','trim|required');
		 $this->form_validation->set_rules('basicpay','Basic Pay','numeric|trim|max_length[6]');
		$this->form_validation->set_rules('phone','Phone','numeric|trim|max_length[14]|min_length[11]');
		$this->form_validation->set_rules('bankaccountno','Bank Account Number','numeric|required|trim|max_length[16]');
		$this->form_validation->set_rules('nic','CNIC','alpha_dash|trim|required|max_length[15]');
		...
		$this->form_validation->set_rules('branchname','Branch Name','trim|required|max_length[50]');
...
public function mfpdb_list() {
	...
	public function dsodb_save() {
		...
		$this->form_validation->set_rules('nic','CNIC','alpha_dash|trim|max_length[15]');
		$this->form_validation->set_rules('present_address','Present Address','alpha_spaces|trim|max_length[100]');
		$this->form_validation->set_rules('passingyear','Passing year','numeric|trim|max_length[4]');
		$this->form_validation->set_rules('branchcode','Branch Code','numeric_spaces|trim|max_length[5]');
		$this->form_validation->set_rules('branchname','Branch Name','alpha_spaces|trim|max_length[50]');
	if ($this->form_validation->run() === FALSE) 
	{
		$edit =  $this -> input -> post('edit');
			if($dsocode!='' && $edit!=''){
		...
		if($this -> input -> post ('dsocode')){
			...
			$data = $this -> System_setup_model -> dsodb_save($dsoData,$dsoCode);
		}else{
	...
	public function med_technician_save() {
		...
		if ($this->form_validation->run() === FALSE) 
		{
			...
			if($this -> input -> post ('techniciancode')){
				...
				$data = $this -> System_setup_model -> med_technician_save($technicianData,$technicianCode);
			}else{
	...
	public function cc_mechanic_save() {
		...
		if ($this->form_validation->run() === FALSE) 
		{
			...
			if($ccm_code!=''){
				...
				//echo '<pre>';print_r($coData);echo '</pre>';exit();
				$data = $this -> System_setup_model ->cc_mechanic_save($ccmData,$ccm_code);
			}else{
		...
		if ($this->form_validation->run() === FALSE) 
		{
			...
			if($go_code!=''){
				...
				$goData = array(
					...
					//print_r($ccmData);exit;
				//echo '<pre>';print_r($coData);echo '</pre>';exit();
				$data = $this -> System_setup_model ->go_save($goData,$go_code);
			}else{
		...
		if ($this->form_validation->run() === FALSE) 
		{
			...
			if($cco_code!=''){
				...
				$ccoData = array(
					...
					//print_r($ccmData);exit;
				//echo '<pre>';print_r($coData);echo '</pre>';exit();
				$data = $this -> System_setup_model ->cco_save($ccoData,$cco_code);
			}else{
		...
		if ($this->form_validation->run() === FALSE) 
		{
			...
			if($cc_techniciancode!=''){
				...
				//print($cc_technicianData);exit;
					//print_r($ccmData);exit;
				//echo '<pre>';print_r($coData);echo '</pre>';exit();
				$data = $this -> System_setup_model ->cc_technician_save($cc_technicianData,$cc_techniciancode);
			}else{
