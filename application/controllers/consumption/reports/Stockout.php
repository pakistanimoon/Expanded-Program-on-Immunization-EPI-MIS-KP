...
class Stockout extends CI_Controller {
	...
	function index(){
		...
		$reportPath = base_url()."consumption/reports/stockout/preview";
		$reportTitle = 'Facilities Stockout Report Filters';
		...
		$dataHtml .= $this->reportfilters->createReportFilters(true,false,false,false,$reportPeriod,false,NULL,NULL,"No","No",NULL,array($reporttypearr,$reporttypeind));
		$dataHtml .= $this->reportfilters->filtersFooter();
		$data['listing_filters'] = $dataHtml;
		$data['data']=$data;
		$data['fileToLoad'] = 'consumption/reports/stockout/filters';
		$data['pageTitle']='Facilities Stockout Report';
	...
	function preview()
	{
		...
		if($this -> input -> post('export_excel'))
		{
			...
			header("Content-Disposition: attachment; filename=EPI_Stockout_Facilities_Report.xls");
			header("Pragma: no-cache");
			header("Expires: 0");
		}
		if(isset($data['report_indicator']) && $data['report_indicator']=="1"){
			if(isset($data["report_type"]) && $data["report_type"]=="2"){
				$title = "Facilities Stock Position (in Doses)";
			}else{
		...
		$dataToReturn['fileToLoad'] = 'consumption/reports/stockout/preview';
