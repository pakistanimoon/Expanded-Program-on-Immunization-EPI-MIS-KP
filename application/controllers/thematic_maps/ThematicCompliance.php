...
class ThematicCompliance extends CI_Controller {
	...
	public function DistrictWiseMapData(){
		...
		$totalComp = ($totalSub/$totalDue)*100;
		$timelyness = ($totalSub >0)?($totaltimely/$totalDue)*100:0;
		...
		{	
			...
			for($i=$month;$i<=$monthto;$i++)
			{
				...
				if($data['compType']=="Consumption" && $code==""){
					$query.="(select duem$i  from consumptioncompliance where consumptioncompliance.distcode= d1.distcode and consumptioncompliance.year='$year') as due{$ind},(select tsubm$i from consumptioncompliance where consumptioncompliance.distcode= d1.distcode and consumptioncompliance.year='$year')as timely{$ind},(select subm$i  from consumptioncompliance where consumptioncompliance.distcode= d1.distcode and consumptioncompliance.year='$year') as sub{$ind},";
				}
				else {
					if($i<10)
					$i="0{$i}";
			...
			foreach($result1 as $row){
				...
				for($i=$month; $i<=$monthto; $i++)
				{
					...
					$totdue = intval($totdue)+intval($due);
					$totsub = intval($totsub)+intval($sub);
					$tottimely = intval($tottimely)+intval($timely);
				}
				$result[$u]['due'] = $totdue;
				$result[$u]['sub'] = $totsub;
