<?php
class ThematicCoronaCountEPID extends CI_Controller {
	...
	public function index(){
		...
		if($this -> input -> get('code')){
			...
			$data['disease'] = ($this -> input -> post('disease'))?$this -> input -> post('disease'):'Covid';
		...
		if($this -> session -> District || $this -> input -> post('id')){
			...
			$data['district'] = $LabelName = $this->session->loginfrom;
			...
			$data['heading']['mapName'] = $data['heading']['barName'] = "Suspected ".ucfirst($data['disease'])." Cases, {$LabelName} Year-{$year} {$con} {$investigationResultText}";
		...
		$viewData['fileToLoad'] = 'thematic_maps/thematic_corona_count_epid';
		$viewData['pageTitle']='EPI-MIS Dashboard | Corona EPID Count ';
	...
	public function onClickUcWiseMapData(){ 
		...
		if($this -> session -> District || $this -> uri -> segment(4)){
			...
			$viewData['filterowbtn'] = 'ThematicCoronaCountEPID';		
			$viewData['fileToLoad'] = 'thematic_maps/thematic_corona_count_epid';
			$viewData['pageTitle']='EPI-MIS Dashboard | Corona EPID Count ';
			$this->load->view('thematic_template/thematic_template',$viewData);
	...
	public function getQuerySelectPortion($data, $indicators=false){
		...
		$table = 'corona_case_investigation_form_db';
		$case_type = $disease;
		//print_r($data); exit;
		$case_type = ($case_type != '0')?" AND case_type = '{$case_type}' ":"";
		// zsk $gender = ($table == 'weekly_vpd')?'':'patient_';
		...
		switch ($data['gender']) {
			...
			case '1':
				...
				break; 
			default:
				$genderType = "(gender = '0' OR gender = '1')";
		...
		if($disease != 'all'){
			...
			$query .= " ((SELECT COUNT(*) FROM corona_case_investigation_form_db WHERE {$genderType}  {$fweek} {$investigationResult} {$part})+(SELECT COUNT(*) FROM corona_case_investigation_form_db WHERE {$genderType} {$fweek} {$investigationResult} {$part})+(SELECT COUNT(*) FROM corona_case_investigation_form_db WHERE {$genderType} {$fweek} {$investigationResult} {$part})) as \"total\" ";
		}
		//	echo $query; exit;
		if($indicators){
			return $dataArray;
	...
	public function getIndicatorData($data){
		...
		if($disease != 'all'){
			//echo 'a';exit; 
			$query .= " (SELECT COUNT(*) FROM {$table} WHERE gender = '1' {$subQuery} {$investigationResult} {$code}) as \"mtotal\",
						(SELECT COUNT(*) FROM {$table} WHERE gender = '0' {$subQuery} {$investigationResult} {$code}) as \"ftotal\", 
						(SELECT COUNT(*) FROM {$table} WHERE (gender = '1' OR gender = '0') {$code} {$subQuery} {$investigationResult}) as \"alltotal\" ";
						
						//print_r($query);exit;
		}else{
			//echo 'b';exit;
			$query .= " ((SELECT COUNT(*) FROM weekly_vpd WHERE gender = '1' AND ( {$subQuery2} )+(SELECT COUNT(*) FROM corona_case_investigation_form_db WHERE {$gender}gender = '1' {$fweek} {$code})+(SELECT COUNT(*) FROM corona_case_investigation_form_db WHERE {$gender}gender = '1' {$fweek} {$investigationResult} {$code})) as \"mtotal\",
		((SELECT COUNT(*) FROM weekly_vpd WHERE gender = '0' AND ( {$subQuery2} )+(SELECT COUNT(*) FROM corona_case_investigation_form_db WHERE {$gender}gender = '0' {$fweek} {$code})+(SELECT COUNT(*) FROM corona_case_investigation_form_db WHERE {$gender}gender = '0' {$fweek} {$investigationResult} {$code})) as \"ftotal\",
				((SELECT COUNT(*) FROM weekly_vpd WHERE (gender = '1' OR gender = '0') AND ( {$subQuery2} )+(SELECT COUNT(*) FROM corona_case_investigation_form_db WHERE ({$gender}gender = '1' OR {$gender}gender = '0') {$fweek} {$code})+(SELECT COUNT(*) FROM corona_case_investigation_form_db WHERE ({$gender}gender = '1' OR {$gender}gender = '0') {$fweek} {$investigationResult} {$code})) as \"alltotal\" ";
		}
	...
	public function ucSummary($parametersData){
		...
		$data['weeklyOutBreakAFP'] = $this -> maps -> weeklyTrendforOut_breakReports(NULL,'corona_case_investigation_form_db',$parametersData['year'],$parametersData['distcode'],NULL,$parametersData['uncode']);
	...
	public function ucSurveillance($parametersData){
		...
		$data['weeklyOutBreakAFP'] = $this ->getChartData("corona_case_investigation_form_db",$parametersData,"AFP","#f3e83a");
