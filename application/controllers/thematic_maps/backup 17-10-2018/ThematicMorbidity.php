...
class ThematicMorbidity extends CI_Controller {
	...
	public function index(){
		...
		$data['disease'] = ($this -> input -> post('disease'))?$this -> input -> post('disease'):'1';
		$data['gender']  = (($this -> input -> post('gender')) || $this -> input -> post('gender') == '0')?$this -> input -> post('gender'):'Both';
		$data['year']  = ($this -> input -> post('year'))?$this -> input -> post('year'):date('Y',strtotime("first day of previous year"));
		...
		if($this -> session -> District || $this -> input -> post('id')){
			...
			$data['heading']['mapName'] = $data['heading']['barName'] = "UC wise ".ucfirst($data['casetype'])." Morbidity, {$data['district']} for year-{$data['year']}";
			...
			$data['heading']['mapName'] = $data['heading']['barName'] = "District Wise ".ucfirst($data['casetype'])." Morbidity for year-{$data['year']}";
	...
	public function onClickUcWiseMapData(){
		if($this -> uri -> segment(4)){
			...
			$data['district'] = get_District_Name($data['id']);
			$data['year']  = ($this -> uri -> segment(5))?$this -> uri -> segment(5):date('Y',strtotime("first day of previous year"));
			...
			$data['casetype']  = ($this -> uri -> segment(9))?$this -> uri -> segment(9):'cases';
			$data['month'] =  ($this -> input -> post('month'))?$this -> input -> post('month'):date('m',strtotime("first day of previous months"));
			$data['special'] = "1";
		}
		//print_r($data);exit;
	...
	public function getQuerySelectPortion($data, $indicators=false){
		...
		if($disease == 'all'){
			...
			$query .= " ((SELECT {$fetch} FROM zero_report zr
					...
					WHERE year = '{$year}' {$part} {$wc} ) ) as total ";
			}
	...
	public function getIndicatorData($data){
		...
		$procode = $this -> session -> provincename;
		$wc .= "AND procode = '{$procode}'";
		$query = "SELECT {$cases} as mtotal, {$deaths} as ftotal, {$deaths} as alltotal from zero_report WHERE year = '{$year}' {$wc}";
