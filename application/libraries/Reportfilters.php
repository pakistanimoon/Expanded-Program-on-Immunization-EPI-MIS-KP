...
class Reportfilters {
	 ...
	 * @return	void
	 */		
	function createReportFilters($district=false, $tehsil=false, $uc=false, $hf=false, $reportPeriod=NULL, $reportType=false, $indicators=NULL, $extraNeededFilters=NULL, $advanceReport="No", $summaryReport="No", $typeWiseReport=NULL, $customDropdown=NULL, $retiredHRreport="No" , $techniciancode=false,$hrtype=NULL, $vaccinationType=NULL, $vaccinationTo=NULL, $agewise=NULL)
	{																	
		if($district == true)
			$this->districtFilter();
		if($tehsil == true)
		...
		if($techniciancode == true)
			$this->technicianFilter();
		if($hrtype)
			$this->hrtype($hrtype);
		if(!(is_null($vaccinationType)))
			$this->vaccinationType();
		if(!(is_null($vaccinationTo)))
			$this->vaccinationTo();
		if(!(is_null($agewise)))
			$this->agegroups();
	...
	function extraNeededFilters($extraNeededFilters){
		if(in_array("caseType",$extraNeededFilters)){
			...
			$this->db->order_by('type_case_name asc');
	...
	function districtFilter()
	{
		...
		if($this->ulevel=='2' OR $this->ulevel=='99')
		{
		...
		{
			$this->options[0] = "--Select District--";
			$this->db->where('divcode',$this->ci->session->Division);
		}
		if($this->ulevel=='3' OR $this->ulevel=='4' OR $this->ulevel=='6'){
	...
	function tehsilFilter()
		...
		if($this->ulevel=='4'){
			...
			if($this->ulevel=='3')
				$this->db->where('distcode',$this->ci->session->District);
			if($this->ulevel=='4')
				$this->db->where('tcode',$this->ci->session->Tehsil);
	...
	function hfFilter()
	{
		...
		if($this->ulevel=='2'){
			...
			if($this->ulevel=='4'){
				$this->db->where(array('tcode' => $this->ci->session->Tehsil, 'hf_type' => 'e' , 'is_ds_fac'=>'1'));
			}
	...
	function technicianFilter()
	{
		...
		if($this->ulevel=='2'){
			...
			$this->options[0] = "--Select Technician--";
			$this->result = $this -> db -> query("SELECT a.code as techniciancode, a.name as technicianname from (SELECT DISTINCT ON (code) code, name, post_hr_sub_type_id, post_distcode, post_status FROM hr_db_history ORDER BY code DESC, id DESC) as a where post_distcode='{$this->ci->session->District}' and post_hr_sub_type_id='01' and post_status='Active' order by name asc") -> result_array();
			/* $this->db->select('code as techniciancode,name as technicianname');
			$this->db->where(array('post_status' => 'Active', 'post_hr_sub_type_id' => '01'));
			if($this->ulevel=='3')
				$this->db->where(array('post_distcode' => $this->ci->session->District));
			$this->db->order_by('name asc');
			$this->result = $this->db->get('hr_db_history')->result_array(); */
			foreach($this->result as $key => $val){
				$this->options[$val['techniciancode']] = $val['technicianname'];
			}
		}
		$this->otherAttributes = 'id="techniciancode" class="form-control"';
		$this->filterRowCreation("Technician", "techniciancode", $this->otherAttributes, $this->options, "dropdown");
	}
	...
	function reportPeriod($reportPeriod){
		// END TESTING                                   
		if(in_array("specific_date_test",$reportPeriod)){    
			$this->options = array('name' => 'datefrom', 'id' => 'monthfrom', 'class' => 'form-control dpcurr', 'required' => 'required','data-date-start-date' => "-2y",'data-date-format' => "yyyy-mm-dd");
			$this->filterRowCreation("Date From", "datefrom", $this->otherAttributes, $this->options, "input");
			$this->options = array('name' => 'dateto', 'id' => 'dateto', 'class' => 'form-control dpcurr', 'required' => 'required','data-date-end-date' => "+0d",'data-date-format' => "yyyy-mm-dd");
		...
		if(in_array("dates",$reportPeriod)){
			$this->options = array('name' => 'datefrom', 'id' => 'datefrom', 'class' => 'form-control dp', 'required' => 'required','data-date-end-date' => "+0d",'data-date-format' => "yyyy-mm-dd");
			$this->filterRowCreation("Date From", "datefrom", $this->otherAttributes, $this->options, "input");
			$this->options = array('name' => 'dateto', 'id' => 'monthto', 'class' => 'form-control dpcurr', 'required' => 'required','data-date-end-date' => "+9m",'data-date-format' => "yyyy-mm-dd");
			$this->filterRowCreation("Date To", "dateto", $this->otherAttributes, $this->options, "input");
		...
		if(in_array("specific_year",$reportPeriod) OR array_key_exists("specific_year",$reportPeriod)){
			$this->options = array('name' => 'year', 'id' => 'year', 'class' => 'form-control dp-year','data-date-end-date' => "+0m");
			if(isset($reportPeriod["specific_year"])){
				$innerarray = $reportPeriod["specific_year"];
				$title = (isset($innerarray["title"]))?$innerarray["title"]:"Year";
				$for = (isset($innerarray["for"]))?$innerarray["for"]:"year";
				$this->filterRowCreation($title, $for, $this->otherAttributes, $this->options, "input");
			}else{
				$this->filterRowCreation("Year", "year", $this->otherAttributes, $this->options, "input");
			}
		}
		...
		if(in_array("month-from",$reportPeriod)){
		...
		if(in_array("compliance_year",$reportPeriod)){
			//$this->options[0] = "--Select Year--";
			$years = $this->getYearsOptions();
			$this->otherAttributes = 'id="compliance_year" class="form-control"';
			$this->filterRowCreation("Year", "year", $this->otherAttributes, $this->options, "dropdown");
		}
		if(in_array("monthfrom",$reportPeriod)){
			if(in_array("compliance_year",$reportPeriod))
				$this->options[0] = "--Select Month--";
			//$months = $this->getMonthsOptionstillLast();
			$this->otherAttributes = 'id="start_month" class="form-control"';
			$this->filterRowCreation("Month From", "monthfrom", $this->otherAttributes, $this->options, "dropdown");
		}
		if(in_array("monthto",$reportPeriod)){
			if(in_array("compliance_year",$reportPeriod))
				$this->options[0] = "--Select Month--";
			//$months = $this->getMonthsOptionstillLast();
			$this->otherAttributes = 'id="end_month" class="form-control"';
			$this->filterRowCreation("Month To", "monthto", $this->otherAttributes, $this->options, "dropdown");
		}
	}
	...
	function reportType()
	{
		...
		$this->options["techniciancode"] ="Technician Wise";
	...
	/**
	 * Create Vaccination Type Filter for Coverage Reports
	 *
	 * @access	public
	 * @return	void
	 */
	function vaccinationType()
	{
		...
		$this->options["fixed"] = "Vaccinator (Fixed)";
		$this->options["outreach"] = "Vaccinator (Outreach)";
		$this->options["mobile"] = "Vaccinator (Mobile)";
		$this->options["lhw"] = "Health House (LHW)";
		$this->otherAttributes = 'id="vaccinationType" class="filter-status form-control"';
		$this->filterRowCreation("Vaccinated by:", "vaccination_type", $this->otherAttributes, $this->options, "dropdown");
	}
	// --------------------------------------------------------------------
	
	/**
	 * Create Vaccination To Filter for Coverage Reports
	 *
	 * @access	public
	 * @return	void
	 */
	function vaccinationTo()
	{
		$this->options["total_children"] = "Total Children";
		$this->options["gender"] = "Gender Wise";
		$this->otherAttributes = 'id="vacc_to" class="filter-status form-control"';
		$this->filterRowCreation("Vaccination To", "vacc_to", $this->otherAttributes, $this->options, "dropdown");
	}
	// --------------------------------------------------------------------
	
	/**
	 * Create Age Group Wise Filter for Coverage Reports
	 *
	 * @access	public
	 * @return	void
	 */
	...
	{
		$this->options["all"] = "--ALL--";
		$this->options["0to11"] = "0-11 Months";
		$this->options["12to23"] = "12-23 Months";
		$this->options["above2"] = "2 years and above";
		$this->otherAttributes = 'id="age_wise" class="filter-status form-control"';
		$this->filterRowCreation("Age Group", "age_wise", $this->otherAttributes, $this->options, "dropdown");
   
	}
	function hrtype($hrtype)
	{
		$this->db->select('*');
		...
		//print_r($result);exit;
		if(in_array("allhrtype",$hrtype)){
			$this->options[0] = "-- ALL --";
			foreach($result as $key => $val){
				$this->options[$val['type_id']] = $val['title']; 
		    }
			$this->otherAttributes = 'id="type_id" class="form-control"';
			$this->filterRowCreation("HR Type", "type_id", $this->otherAttributes, $this->options, "dropdown");
		}	
		if(in_array("hrtype",$hrtype)){
			foreach($result as $key => $val){
				$this->options[$val['type_id']] = $val['title']; 
		    }
			$this->otherAttributes = 'id="type_id" class="form-control"';
	...
	function filterRowCreation($title=NULL, $for=NULL, $attr=NULL, $opt=NULL, $type=NULL, $classes=NULL,$modId=Null){
		...
		if($type == "checkbox"){
			...
			$this->outputHtml .= form_checkbox($dataArray);								  
		}
		$this->outputHtml .='  					
	...
	function customDropdown($customDropdown)
	{
		foreach($customDropdown as $dropdown){
			if(isset($dropdown['type']) && $dropdown['type']=='checkbox'){
				$type = 'checkbox';
				unset($dropdown['type']);
			}else{
				$type = 'dropdown';
			}
			if(isset($dropdown['type']))
				unset($dropdown['type']);
			...
			$this->filterRowCreation($nameforDropDown, str_replace(" ","_",strtolower($nameforDropDown)),$this->otherAttributes, $this->options, $type,$classforHidding);
		}
	}
	...
	function getCRYearsOptions(){
		...
		/* if($WeekNumber > 01){
			...
			$this->db->where('year <=',$currentYear);//date('Y')
		//}
