...
class Ajax_calls_model extends CI_Model {
	...
	public function med_technician_filter($page, $distcode, $tcode, $facode, $uncode, $supervisorcode, $status, $fatype) {
		...
		$wc = getWC_Array('3',$distcode,$facode);
		if ($tcode != 0) {
			$wc[] = "tcode = '$tcode' ";
		}
		if ($facode != 0) {
	...
	public function getSupervisor () {
		...
		$distcode = $this->session->District;
		$query = "select supervisorcode as supervisorcode, supervisorname as supervisor  from supervisordb where  distcode='$distcode' And supervisor_type='$supervisor_type' AND status='Active' ";
		$result = $this -> db -> query($query);
		$result = $result -> result_array();	
	    $data = '<option value="">--Select--</option>';
	...
	public function facility_filter($page, $tcode, $fatype) {
		$procode = isset($_REQUEST['procode']) ? $_REQUEST['procode'] : $_SESSION['Province'];
		//$distcode = isset($_REQUEST['distcode']) ? $_REQUEST['distcode'] : $_SESSION['District'];
		if($this -> session -> District){
			$distcode = $this -> session -> District;
	...
	public function checkCodes($supervisornewcode,$supervisorcode, $techniciancode, $facodew, $distcodedriver, $distcodeas,$distcodeco,$distcodeccm,$distcodego,$distcodecco,$distcodecct,$facodecct,$chkdsocode,$dsocode,$chkcctcode,$cctcode,$chkccmcode,$ccmcode, $chkccgcode,$ccgcode, $chkccdcode,$ccdcode, $facodem,$deodcode,$skdcode) {
...
public function codb_filter($page,$status,$distcode){
	...
	public function checktechNIC($nic,$code){
		...
		if($code!=''){
			...
			$query = "SELECT nic FROM techniciandb WHERE nic='".$nic."'"; 
		}
		$query= $this->db->query($query);
		$result = $query->row_array();
			if ($result==true)
...
//----------------------------------------Measles LINE LIST FILTER--------------------------------------------------//
	...
	public function zero_reporting_filter($year,$week) {
		...
		foreach ($result as $row) {
		    ...
		    <td class="text-left">' . $row['week'] . '</td>
	...
	public function createReport($rep_title,$fIds,$lastsgment){
	...
	if($rep_title=="")
	{
		...
		if($fIds=="")
		{
			...
			//to add report title
			$insertquery = "INSERT INTO adv_reports (username, report_title, module_id ) VALUES ('$username', '$rep_title', '$lastsgment')";
	...
	$body .='<div class="row">
		...
		<div class="col-md-5 col-md-offset-5 col-sm-6 col-sm-offset-6 col-xs-6 col-xs-offset-6">
			...
			<a class="btn btn-success" href="#" name="closeReportAddBtn" id="closeReportAddBtn" data-sec="'.$secId.'"><i class="fa fa-times"></i> Cancel</a>
	...
	public function moonzerorepcomp($year,$distcode){
		...
		$query = "select $monthlyPortion from districts join (select distcode,count(facilities.facode) as due01 from facilities where hf_type='e' group by distcode ) as inntable on inntable.distcode = districts.distcode where districts.distcode = '".$distcode."' ORDER BY districts.district";
		$result = $this -> db -> query($query);
