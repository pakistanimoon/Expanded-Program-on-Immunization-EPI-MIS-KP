...
class Compliances_model extends CI_Model {
	...
	function HFMVRF($data,$title){
		...
		if(array_key_exists("distcode", $data) && $data['distcode'] > 0){
			...
			//==========================================			
			$queryForYearlyData .= "(select count(facode) from fac_mvrf_db where fmonth like '$year-%' and facode = flcf1.facode and submitted_date>= '".$year."-02-01' and extract(day from submitted_date)<=5 ) AS Timeliness,";
			...
			for ($ind = 1; $ind < 13; $ind++) {
				...
				if(($year == $currYear) && ($ind >= $currMonth)){}else{		
					...
					$monthlyPortion .= "(select count(fac.facode)  from facilities fac where  fac.distcode = districts.distcode and fac.hf_type='e' and fac.is_vacc_fac='1') AS  due$ind ,

										(select count(fac_mvrf_db.facode)  from fac_mvrf_db join facilities fac on fac.facode = fac_mvrf_db.facode where fac_mvrf_db.fmonth = '$year-$ind' and fac_mvrf_db.distcode = districts.distcode and fac.hf_type='e' and fac.is_vacc_fac='1' ) AS  sub$ind,
										(select count(fac_mvrf_db.facode) 
								from fac_mvrf_db join facilities fac on fac.facode = fac_mvrf_db.facode 
										where fac_mvrf_db.submitted_date >= '{$nextyear}-{$nextmonth}-01' and fac_mvrf_db.submitted_date <= '{$nextyear}-{$nextmonth}-05' and fac_mvrf_db.distcode = districts.distcode and fac.hf_type='e' and fac.is_vacc_fac='1') AS timely$ind,
										";
					$outerPortion .= "due$ind,sub$ind,round((sub$ind::float//due$ind)::numeric*100,0) as \"%$ind\",timely$ind,round((timely$ind::float//due$ind)::numeric*100,0) as \"Timely%$ind\",";

					$allouterPortion .= "sum(due$ind) as totalDue$ind,sum(sub$ind) as totalSub$ind, round((sum(sub$ind)::float//sum(due$ind))::numeric*100,1) as TotalPerc$ind,sum(timely$ind) as totalTimely$ind,round((sum(timely$ind)::float//sum(due$ind))::numeric*100,1) as TotaltimelyPerc$ind,";
				}
			}			
			$monthlyPortion .= "(Select CASE WHEN  count(fac.facode) IS NULL THEN 0 ELSE round((count(fac.facode))::numeric*".count($headerArray).",0) END from facilities fac where fac.distcode = districts.distcode and fac.hf_type='e') as totaldue,
								...
								(Select count(fac_mvrf_db.facode)  from fac_mvrf_db join facilities fac on fac.facode = fac_mvrf_db.facode where TRIM(BOTH ' ' FROM extract(day from fac_mvrf_db.submitted_date)::text)::integer >= 1 and TRIM(BOTH ' ' FROM extract(day from fac_mvrf_db.submitted_date)::text)::integer <= 5 and fac_mvrf_db.distcode = districts.distcode and fac_mvrf_db.fmonth like '$year-%' ) as totaltimely
								";
			$outerPortion .= "totaldue as due$ind,totalsub as sub$ind,round((totalsub::float//totaldue)::numeric*100,1) as \"%$ind\",totaltimely as timely$ind,round((totaltimely::float//totaldue)::numeric*100,1) as \"Timely%$ind\"";
				  $allouterPortion .= "sum(due$ind) as totalDue$ind,sum(sub$ind) as totalSub$ind, round((sum(sub$ind)::float//sum(due$ind))::numeric*100,1) as TotalPerc$ind,sum(timely$ind) as totalTimely$ind,round((sum(timely$ind)::float//sum(due$ind))::numeric*100,1) as TotalTimelyPerc$ind"; 
			...
			$query = 'select distcode, district, ' . $outerPortion . ' from (' . $query . ') as a'; 
			 $result = $this -> db -> query($query); //print_r($result); exit;
	...
	function Measles_Compliance($data,$title){
		...
		if(array_key_exists("distcode", $data) && $data['distcode'] > 0){
			...
			$monthlyPortion .= "(select sum(measle_cases) from zero_report where fweek like '$year-%' and facode = facilities.facode and distcode = '$distcode' ) AS  totaldue,
								(select count(mes.facode)  from case_investigation_db mes where mes.case_type='Msl' AND mes.fweek like '$year-%' and mes.facode = facilities.facode and distcode = '$distcode' ) as totalsub";
			if($week <= 0)
				$outerPortion .= "totaldue as due$ind,totalsub as sub$ind, round((totalsub::float//totaldue)::numeric*100,1) as \"%$ind\"";
			$allouterPortion .= "sum(due$ind) as totalDue$ind,sum(sub$ind) as totalSub$ind, round((sum(sub$ind)::float//sum(due$ind))::numeric*100) as TotalPerc$ind";
			//month wise end
		...
		if($week <= 0){
			...
			$monthlyPortion .= "(select sum(measle_cases) from zero_report where fweek like '$year-%' and distcode = districts.distcode ) AS  totaldue,
								(select count(mes.facode)  from case_investigation_db mes where mes.case_type='Msl' AND mes.fweek like '$year-%' and mes.distcode = districts.distcode ) as totalsub";
			if($week <= 0)
				$outerPortion .= "totaldue as due$ind,totalsub as sub$ind, round((totalsub::float//totaldue)::numeric*100,1) as \"%$ind\"";
			$allouterPortion .= "sum(due$ind) as totalDue$ind,sum(sub$ind) as totalSub$ind, round((sum(sub$ind)::float//sum(due$ind))::numeric*100) as TotalPerc$ind";
			//month wise end
	...
	function Zero_Compliance($data,$title){
		...
		if(array_key_exists("distcode", $data) && $data['distcode'] > 0){
			...
			if($data['year'] == date('Y')){
				...
				$query = "select $monthlyPortion from facilities where facilities.hf_type='e' and is_ds_fac='1' limit 1";
