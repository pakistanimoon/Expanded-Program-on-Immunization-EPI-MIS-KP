...
class Cerv_reports_model extends CI_Model {
	...
	function CERV_PVR($data){
		$whereFmonth = " submitted_date::text BETWEEN '" . $data['monthfrom'] . "-01' AND '" . date("Y-m-t", strtotime($data['monthto'])) . "'";		
		$whereCondition = "";
		...
		if(isset($data['distcode']) && $data['distcode'] > 0){
			$query = "
				SELECT 
					...
					epi_childreg.penta1,epi_childreg.penta2,epi_childreg.penta3,epi_childreg.pcv10_1,epi_childreg.pcv10_2,epi_childreg.pcv10_3,
		...
		$subTitle = "Permanent Vaccination Register Report";
