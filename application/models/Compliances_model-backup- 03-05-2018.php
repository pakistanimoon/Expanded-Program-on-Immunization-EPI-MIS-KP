	...
	function HF_Consumption_Requisition($data,$title){
		...
		if(array_key_exists("distcode", $data) && $data['distcode'] > 0){
			...
			for ($i; $i < 13; ) {
				...
				{
					$asValueHead=$headNames[$i-1];
					$queryForYearlyData .= "CASE WHEN CAST((select facode  from form_b_cr where fmonth = '$year-$mon' and facode = flcf1.facode  and getfstatus_vacc('$year-$mon',facode) = 'F') AS INTEGER) > 0 THEN 
											(CASE WHEN CAST((select facode from form_b_cr where fmonth = '$year-$mon' and date_submitted>='$datefrom' and date_submitted<='$date' and facode = flcf1.facode  and getfstatus_vacc('$year-$mon',facode) = 'F') AS INTEGER) > 0 THEN 'timely' ELSE 'complete' END)
											ELSE 'notsubmitted' END AS $asValueHead, ";
					$allTotalPortion .= "sum(CASE (" . $headNames[$i - 1] . ") WHEN ('notsubmitted') THEN 0 ELSE (CASE (" . $headNames[$i - 1] . ") WHEN ('complete') THEN 0 ELSE 1 END) END) as timely$i,";
					$allTotalPortion .= "sum(CASE (" . $headNames[$i - 1] . ") WHEN ('notsubmitted') THEN 0 ELSE 1 END) as complete$i,";
					$allTotalPortion .= "(select get_monthly_fstatus_vacc('$year-$mon','{$data['distcode']}')) as total$i,";
			...
			$queryForYearlyData .= "(select count(facode) from form_b_cr where fmonth like '$year-%' and facode = flcf1.facode and date_submitted >= '".$year."-02-01' and extract(day from date_submitted)<=5 and extract(Month from date_submitted - interval '1 month')::integer=substring(fmonth from 6 for 2)::integer) AS Timeliness,";
			...
			$query = 'select distcode, district, ' . $outerPortion . ' from (' . $query . ') as a';
			$result = $this -> db -> query($query);
			$data['allData'] = $result -> result_array();//print_r($data['allData']);exit;
			//for vertical total of all rows.
			$queryForTotal = 'select ' . $allouterPortion . ' from (' . $query . ') as b';
			$resultTotal = $this -> db -> query($queryForTotal);
			$data['allDataTotal'] = $resultTotal -> result_array();
