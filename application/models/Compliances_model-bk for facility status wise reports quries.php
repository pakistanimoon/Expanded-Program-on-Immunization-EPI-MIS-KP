...
class Compliances_model extends CI_Model {
	...
	function HFMVRF($data,$title){
		...
		if(array_key_exists("distcode", $data) && $data['distcode'] > 0){
			...
			foreach ($result as $key => $value) {
				$percentageTotalT = ($value['timeliness']/$value['total'])*100;
				$percentageTotalC = ($value['completeness']/$value['total'])*100;
			...
			foreach ($data['allDataTotal'] as $index => $value_arr) {
				$percentageTotalTime = round(($value_arr['timeliness']/$value_arr['total'])*100,2);
				$percentageTotalComp = round(($value_arr['completeness']/$value_arr['total'])*100,2);			
	...
	function HF_Consumption_Requisition($data,$title){
		...
		if(array_key_exists("distcode", $data) && $data['distcode'] > 0){
			...
			$queryForYearlyData .= "(select count(facode) from form_b_cr where fmonth like '$year-%' and facode = flcf1.facode and date_submitted>= '".$year."-01-01' and extract(day from date_submitted)<=5 and extract(Month from date_submitted)::integer=substring(fmonth from 6 for 2)::integer) AS Timeliness,";
			...
			foreach ($result as $key => $value) {
				$percentageTotalT = round(($value['timeliness']/$value['total'])*100,2);
				$percentageTotalC = round(($value['completeness']/$value['total'])*100,2);
				$result[$key]['total timeliness'] = $value['timeliness'].'/'.$value['total'].' ='.$percentageTotalT.'%';
				$result[$key]['total completeness'] = $value['completeness'].'/'.$value['total'].' ='.$percentageTotalC.'%';
				unset($result[$key]['timeliness']);
				unset($result[$key]['completeness']);
...
function Zero_Compliance($data,$title){
		...
		if(array_key_exists("distcode", $data) && $data['distcode'] > 0){
			...
			$data['allDataTotal'] = $resultTotal -> result_array();
			foreach ($result as $key => $value) {
				$timelinessPercentage = ($value['total']>0)?round(($value['timeliness']/$value['total'])*100,2):0;
				$completenessPercentage = ($value['total']>0)?round(($value['completeness']/$value['total'])*100,2):0;
			...
			foreach ($data['allDataTotal'] as $index => $value_arr) {
				$totalTimelinessPercentage = round(($value_arr['timeliness']/$value_arr['total'])*100,2);
				...
				foreach ($value_arr as $key => $value) 
				{
					if(substr($key, 0, 6) == 'timely' AND $key != 'timeliness' )
					{
						$timely = round(($value/$value_arr['total'.$i])*100,2);
						$data['allDataTotal'][$index][$key] = $value.'/'.$value_arr['total'.$i].' = '.$timely.'%';
					}
					if(substr($key, 0, 8) == 'complete' AND $key != 'completeness' )
					{
						$complete = round(($value/$value_arr['total'.$i])*100,2);
				...
				$data['allDataTotal'][$index+1]['timeliness'] = "";
				$totalCompletenessPercentage = round(($value_arr['timeliness']/$value_arr['total'])*100,2);
