	...
	function HF_Consumption_Requisition_Test($data,$title){
		...
		if(array_key_exists("distcode", $data) && $data['distcode'] > 0){
			...
			for( $j=$start_year ; $j <= $end_year ; $j++)
			{
					...
					for ($ind = $start_month; $ind <= $end_month; $ind++)
					{
						...
						else
						{
							...
							(CASE WHEN 
								...
								then
									(CASE WHEN
										exists(select 1 from $table where fmonth = '$start_year-$mon' and facode = flcf1.facode) 
									THEN 
										(CASE WHEN 
											exists(select 1 from $table where fmonth = '$start_year-$mon' and $datcol <= '$submittedYear-$submittedDateMonth-10' and facode = flcf1.facode) 
										THEN 'timely' 
										ELSE 'complete' END) 
									ELSE 'notsubmitted' END) 
								ELSE 'notfunctional'
					...
					for ($ind = 1; $ind <= $end_monthhs; $ind++)
					{
						...
						else
						{
							...
							(CASE WHEN 
								...
								then
									(CASE WHEN
										exists(select 1 from $table where fmonth = '$end_year-$mon' and facode = flcf1.facode) 
									THEN 
										(CASE WHEN 
											exists(select 1 from $table where fmonth = '$end_year-$mon' and $datcol <= '$submittedYear-$submittedDateMonth-10' and facode = flcf1.facode) 
										THEN 'timely' 
										ELSE 'complete' END) 
									ELSE 'notsubmitted' END) 
								ELSE 'notfunctional'
				...
				else // between year
				{
					...
					for ($ind = 1; $ind < 13 ; $ind++)
					{
						...
						(CASE WHEN 
							...
							then
								(CASE WHEN
									exists(select 1 from $table where fmonth = '$between_year-$mon' and facode = flcf1.facode) 
								THEN 
									(CASE WHEN 
										exists(select 1 from $table where fmonth = '$between_year-$mon' and $datcol <= '$submittedYear-$submittedDateMonth-10' and facode = flcf1.facode) 
									THEN 'timely' 
									ELSE 'complete' END) 
								ELSE 'notsubmitted' END) 
							ELSE 'notfunctional'
				...
				if($cummulativeTotal == ""){
					if($j==$start_year){
						$cummulativeTimeliness = " count(facode) from $table where fmonth between '$fmonth1' and '$fmonth2' and facode = flcf1.facode and getfstatus_vacc(fmonth,facode)='F' and $datcol >= TO_DATE(substring(fmonth from 1 for 4)||'-02-01','YYYY-MM-DD') and extract(day from $datcol)<=10 and extract(Month from $datcol - interval '1 month')::integer=substring(fmonth from 6 for 2)::integer ";
		...
		else
		{
			...
			for( $j=$start_year ; $j <= $end_year ; $j++)
			{
				...
				else if($j == $end_year) // end year
				{
					...
					$topHead = array("jan-{$end_year}", "feb-{$end_year}", "mar-{$end_year}", "apr-{$end_year}", "may-{$end_year}", "jun-{$end_year}", "jul-{$end_year}", "aug-{$end_year}", "sep-{$end_year}", "oct-{$end_year}", "nov-{$end_year}", "dec-{$end_year}");
					//$end_monthhs = 0;
					for ($ind = $start_month; $ind <= $end_monthhs; $ind++)
					{
						$col = sprintf("%02d", $col);
						$i=$ind;
						$ind = sprintf("%02d", $ind);
