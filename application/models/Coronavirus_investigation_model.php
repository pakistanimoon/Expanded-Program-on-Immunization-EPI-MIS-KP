<?php
class Coronavirus_investigation_model extends CI_Model {
	...
	public function coronavirus_investigation()
	{
		...
		$this->breadcrumbs->push('Coronavirus Investigation List', '/Coronavirus_investigation/coronavirus_investigation_list');
		//////////////////////////////Adding BreadCrums////////////////////
		$wc = getWC();
	...
	public function coronavirus_investigation_list($per_page,$startpoint)
	{
		...
		//echo $pro_code;exit();
		//print_r($wc);exit;
		$query="SELECT distinct cross_notified_from_distcode, districtname(cross_notified_from_distcode) as cross_notified_from_distname from corona_case_investigation_form_db where $wc and (cross_notified_from_distcode != '0' and cross_notified_from_distcode != '') order by cross_notified_from_distname";
		...
		$query="SELECT distinct year from corona_case_investigation_form_db where $wc and year IS NOT NULL order by year";
		$resultYear=$this -> db -> query($query);
		$data['resultYear'] = $resultYear -> result_array();

		$query="SELECT distinct week from corona_case_investigation_form_db where $wc order by week";
