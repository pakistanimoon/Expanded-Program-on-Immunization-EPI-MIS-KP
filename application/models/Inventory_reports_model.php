...
class Inventory_reports_model extends CI_Model {
	...
	public function getprevbalance($startdate,$whtype,$whcode,$expire_startdate) {
		...
		//query
		$this->db->select("item_pack_size_id as id,sum(case when tt.nature = '1' AND $whererec then batch.quantity else 0 end)-sum(case when tt.nature = '0' AND $whereissue then batch.quantity else 0 end) as balance",FALSE);
...
//echo "$distcode";	
	...
	{
		$this->db->select("sum(case when tt.nature = '1' AND master.to_warehouse_type_id = '".$whtype."' and master.to_warehouse_code='".$whcode."' then batch.quantity else 0 end)-sum(case when tt.nature = '0' AND master.from_warehouse_type_id = '".$whtype."' and master.from_warehouse_code='".$whcode."' then batch.quantity else 0 end) as stock,batch.number",FALSE);
		$this->db->from("epi_stock_batch_history batch"); 
		$this->db->join("epi_stock_master_history master","master.master_id = batch.batch_master_id");
		$this->db->join("epi_transaction_types tt","tt.pk_id = master.transaction_type_id");
		$this->db->join("epi_item_pack_sizes sizes","sizes.pk_id = batch.item_pack_size_id");
