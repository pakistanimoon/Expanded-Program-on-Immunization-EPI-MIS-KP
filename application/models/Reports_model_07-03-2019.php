...
class Reports_model extends CI_Model {
	...
	public function Create_Reporting_Filters($reportName) {
		...
		if ($reportName == 'vaccine_demand') {
			$Caption = "Vaccine Demand Report";
			$datArray['typeWise'] = "";
			$datArray['month-from-to-previous'] = "";
			$datArray['demand_type'] = "";
			$datArray['vaccines'] = "";
			unset($datArray['years']);
		}
...
/**
	...
	public function vaccine_demand($data)
	{
		...
		if(isset($data['distcode']) AND $data['distcode'] > 0 AND $data['type_wise'] == 'facility')
		{
			...
			while ($fmonth <= $data['monthto']) 
			{
				$m_portion .= "(SELECT COALESCE(SUM($column_name), 0) FROM form_b_cr 
								WHERE fmonth='$fmonth' AND form_b_cr.facode=facilities.facode) AS \"$th\",";
			...
			$m_portion .= "(SELECT COALESCE(SUM($column_name), 0) FROM form_b_cr 
								WHERE fmonth BETWEEN '{$data['monthfrom']}' AND '{$data['monthto']}' AND form_b_cr.facode=facilities.facode) AS \"all total\"";
			$t_portion .= "sum(\"all total\") AS \"Total\",";
			$m_portion .= " FROM facilities WHERE distcode='{$data['distcode']}' AND hf_type='e' ORDER BY facode";
		}
		...
		else
		{
			...
			{
				$m_portion .= "(SELECT COALESCE(SUM($column_name), 0) FROM form_b_cr 
								WHERE fmonth='$fmonth' AND form_b_cr.distcode=districts.distcode) AS \"$th\",";
				$t_portion .= "sum(\"$th\") AS \"Total $th\",";
				$th = date('M Y', strtotime($fmonth. 'first day of next month'));
				$fmonth = date('Y-m', strtotime($fmonth. 'first day of next month'));
			}
			$m_portion .= "(SELECT COALESCE(SUM($column_name), 0) FROM form_b_cr 
								WHERE fmonth BETWEEN '{$data['monthfrom']}' AND '{$data['monthto']}' AND form_b_cr.distcode=districts.distcode) AS \"all total\"";
			$t_portion .= "sum(\"all total\") AS \"Total\",";
		...
		$t_portion .= " FROM ($m_portion) As b";
		$data['allDataTotal'] = $this->db->query($t_portion)->result_array();
		$result['htmlData'] = getListingReportTable($data['allData'], '', $data['allDataTotal']);
		$sub_title = "Vaccine Demand Report";
		$result['subtitle'] = $sub_title;
		$data['vacc_demand_product_name'] = $data['vaccine'];
	...
	public function access_utilization($data){
		...
		if(isset($data['distcode']) AND $data['distcode'] > 0 AND $data['type_wise'] == 'Uniouncouncil'){	
	...
	public function measle_coverage_dropout($data)
	{
		...
		if($period_wise == 'monthly')
		{
			...
			for ($i=1; $i <= 12; $i++) 
			{
				...
				$where_fmonth = "fmonth BETWEEN '$year-$month' AND '$year-$month2'";
				$fmonth = "$year-$month2";
				$start_date = date('Y-m-01',strtotime($fmonth.' this month'));
				$end_date = date('Y-m-t',strtotime($fmonth.' this month'));
				//echo $start_date.PHP_EOL;
				//echo $end_date.PHP_EOL;
				foreach ($result as $key => $value) 
				{
					$numenator = check_replacements($value['numenator'],$year,$month,$year,$month2,$type);
					$denominator = check_replacements($value['denominator'],$year,$month,$year,$month2,$type);
					if($value['mt'])
					{
						$mt = $value['mt'];
					}
		...
		elseif($period_wise == 'quarterly')
		{
			...
			for ($i=1; $i <= 4; $i++) 
			{
				...
				$m1 = sprintf('%02d',$m1);
				$m2 = sprintf('%02d',$m2);
				$where_fmonth = "fmonth BETWEEN '$year-$m1' AND '$year-$m2'";
				$fmonth = "$year-$m1";
				$start_date = date('Y-m-01',strtotime($fmonth.' this month'));
				$fmonth = "$year-$m2";
				$end_date = date('Y-m-t',strtotime($fmonth.' this month'));
				foreach ($result as $key => $value) 
				{
					$numenator = check_replacements($value['numenator'],$year,$m1,$year,$m2,$type);
					$denominator = check_replacements($value['denominator'],$year,$m1,$year,$m2,$type);
					if($value['mt'])
					{
						$mt = $value['mt'];
					}
		...
		//For Yearly Total
		$where_fmonth = "fmonth BETWEEN '$t_year1-$t_month1' AND '$t_year2-$t_month2'";
		$fmonth = "$t_year1-$t_month1";
		$start_date = date('Y-m-01',strtotime($fmonth.' this month'));
		$fmonth = "$t_year2-$t_month2";
		$end_date = date('Y-m-t',strtotime($fmonth.' this month'));
		foreach ($result as $key => $value) 
		{
			$numenator = check_replacements($value['numenator'],$t_year1,$t_month1,$t_year2,$t_month2,$type);
			$denominator = check_replacements($value['denominator'],$t_year1,$t_month1,$t_year2,$t_month2,$type);
			if($value['mt'])
			{
				$mt = $value['mt'];
			}
			...
			$m_portion .= "COALESCE((SELECT ROUND( COALESCE( ($numenator) // NULLIF($denominator, 0), 0)::numeric * $mt) FROM fac_mvrf_db WHERE $where_fmonth AND $condition GROUP BY $code), 0) AS \"Yearly {$value['ind_name']} $i\",";
			$sub_query_total .= "(SELECT ROUND($numenator, 2) FROM fac_mvrf_db WHERE $where_fmonth AND $condition) AS \"$num $i\", (SELECT ROUND($denominator, 2) FROM fac_mvrf_db WHERE $where_fmonth AND $condition GROUP BY $code) AS \"$den $i\",";
			$t_portion .= "ROUND( COALESCE( sum(\"$num $i\") // NULLIF(sum(\"$den $i\"), 0), 0)::numeric * $mt) AS \"Total {$value['ind_name']} $i\",";
		}
		$temp_str = "COALESCE((SELECT COUNT(*) FROM case_investigation_db WHERE pvh_date BETWEEN '$start_date'::date AND '$end_date'::date AND case_type='Msl' AND $condition GROUP BY $code),0) AS \"Yearly Cases $i\",";
		$m_portion .= $temp_str;
		$sub_query_total .= $temp_str;
