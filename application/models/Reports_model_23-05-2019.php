...
class Reports_model extends CI_Model {
	...
	public function hf_consumption_requisition($data)
	{
		...
		$datfrom = $date_from[0]-1;
		$datfrom = '0'.$datfrom;
		if($datfrom == 00 )
		{
			$date_toy[1]=$date_from[1]-1;
			$date_tom[0] = $date_from[0]+11;
			$OB_date = $date_toy[1]."-".$date_tom[0];
		...
		{
			$criNames1 = $this -> Common_model -> fetchall('epi_item_pack_sizes','','item_name,number_of_doses,cr_table_row_numb,', array('item_category_id<>'=>4,'activity_type_id'=>1),'',array('by'=>'list_rank','type'=>'asc'));
			$queryForYearlyData="select";	  
			foreach($criNames1 as $key => $val)
			{
				$rowno = $val['cr_table_row_numb'];
				$find = array("-"," ",".","(",")");
				$replace = array("");
				$itemnames = array($val['item_name']);
				 $items_doses[$val['item_name']] = $val['number_of_doses'];
				$itemname= str_replace($find,$replace,$itemnames); 
				$queryForYearlyData.="
				(select sum(cr_r".$rowno."_f1) from form_b_cr where districts.distcode=form_b_cr.distcode and  fmonth = '$new_date_from' ".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).") as ".$itemname[0]."OB,";
				$queryForYearlyData.="
				(select sum(cr_r".$rowno."_f2) from form_b_cr where districts.distcode=form_b_cr.distcode".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).((!empty($wc1)) ?  ' AND ' . implode(' AND ', $wc1) : '' ).") as ".$itemname[0]."R,";
				$queryForYearlyData.="
				(select sum(cr_r".$rowno."_f4) from form_b_cr where districts.distcode=form_b_cr.distcode ".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).((!empty($wc1)) ?  ' AND ' . implode(' AND ', $wc1) : '' ).") as ".$itemname[0]."USE,";
				$queryForYearlyData.="
				(select sum(cr_r".$rowno."_f5) from form_b_cr where districts.distcode=form_b_cr.distcode ".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).((!empty($wc1)) ?  ' AND ' . implode(' AND ', $wc1) : '' ).") as ".$itemname[0]."UNUSE,";
				$queryForYearlyData.="
				(select sum(cr_r".$rowno."_f6) from form_b_cr where districts.distcode=form_b_cr.distcode  and  fmonth = '$new_date_to' ".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).") as ".$itemname[0]."CB,";
	        }
			...
			$queryForYearlyData.=" from districts where distcode = '".$data['distcode']."'";
				...
				foreach($criNames1 as $key => $val)
				{
					...
					$itemname= str_replace($find,$replace,$itemnames); 
					$queryForYearlyData.="
					(select sum(cr_r".$rowno."_f1) from form_b_cr where form_b_cr.procode='".$_SESSION["Province"]."' and  fmonth = '$new_date_from' ".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).") as ".$itemname[0]."OB,";
					$queryForYearlyData.="
					(select sum(cr_r".$rowno."_f2) from form_b_cr where form_b_cr.procode='".$_SESSION["Province"]."' ".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).((!empty($wc1)) ?  ' AND ' . implode(' AND ', $wc1) : '' ).") as ".$itemname[0]."R,";
					$queryForYearlyData.="
					(select sum(cr_r".$rowno."_f4) from form_b_cr where form_b_cr.procode='".$_SESSION["Province"]."'  ".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).((!empty($wc1)) ?  ' AND ' . implode(' AND ', $wc1) : '' ).") as ".$itemname[0]."USE,";
					$queryForYearlyData.="
					(select sum(cr_r".$rowno."_f5) from form_b_cr where form_b_cr.procode='".$_SESSION["Province"]."'  ".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).((!empty($wc1)) ?  ' AND ' . implode(' AND ', $wc1) : '' ).") as ".$itemname[0]."UNUSE,";
					$queryForYearlyData.="
					(select sum(cr_r".$rowno."_f6) from form_b_cr where form_b_cr.procode='".$_SESSION["Province"]."'   and  fmonth = '$new_date_to' ".((!empty($wc)) ?  ' AND ' . implode(' AND ', $wc) : '' ).") as ".$itemname[0]."CB,";
...
/**
	...
	public function vaccine_demand($data)
	{
		...
		if(isset($data['distcode']) AND $data['distcode'] > 0 /*  AND $data['type_wise'] == 'facility' */)
		{
			...
			$moonquery = "select facilities.facode, facilityname(facilities.facode),
			$m_portion
			...
			WHERE facilities.distcode='{$data['distcode']}' AND hf_type='e' AND fmonth BETWEEN '{$data['monthfrom']}' AND '{$data['monthto']}' AND item_id = $product 
			group by facilities.facode";
		...
		else
		{
			...
			while ($fmonth <= $data['monthto']) 
			{
				$m_portion .= "(SELECT COALESCE(SUM($column_name), 0) FROM epi_consumption_detail
								join epi_consumption_master on epi_consumption_master.pk_id = epi_consumption_detail.main_id 
								WHERE fmonth='$fmonth' AND distcode=districts.distcode AND item_id = $product) AS \"$th\",";
				$t_portion .= "sum(\"$th\") AS \"Total $th\",";
				$th = date('M Y', strtotime($fmonth. 'first day of next month'));
				$fmonth = date('Y-m', strtotime($fmonth. 'first day of next month'));
			}
			$m_portion .= "(SELECT COALESCE(SUM($column_name), 0) FROM epi_consumption_detail
								join epi_consumption_master on epi_consumption_master.pk_id = epi_consumption_detail.main_id 
								WHERE fmonth BETWEEN '{$data['monthfrom']}' AND '{$data['monthto']}' AND distcode=districts.distcode AND item_id = $product) AS \"all total\"";
			$t_portion .= "sum(\"all total\") AS \"Total\",";
		...
		$data['allData'] = $this->db->query($moonquery)->result_array();
		...
		$result['data'] = $data;
		return $result;
		//echo "<pre>";print_r($result['htmlData']);exit;
	}
	public function all_dropout($data)
	{
		$query = "SELECT * FROM indicator_main WHERE indmain IN (24,25,26,27) ORDER BY indmain";
	...
	public function access_utilization($data){
		...
		if(isset($data['distcode']) AND $data['distcode'] > 0 ){
			...
			if($data["type_wise"]=="facility"){
				...
				$query="Select * from(select facode,facilityname(facode), 
						...
						sum(case when (Access < 80) and (utilization >= 10) then 1 else null end) as cat4 from(select facode, 
						COALESCE(round(((sumvaccinevacination(7,fac_mvrf_db.facode,'{$fmonthfrom}','{$fmonthto}')::numeric
						/NULLIF(getmonthlytarget_specificyearrsurvivinginfants(fac_mvrf_db.facode,'facility','{$startyear}','{$sm}','{$endyear}','{$em}') :: float,0))*100):: numeric,0),0) as Access, 
						COALESCE(round(((sumvaccinevacination(7,fac_mvrf_db.facode,'{$fmonthfrom}','{$fmonthto}') :: numeric - sumvaccinevacination(9,fac_mvrf_db.facode,'{$fmonthfrom}','{$fmonthto}'):: numeric)/NULLIF(sumvaccinevacination(7,fac_mvrf_db.facode,'{$fmonthfrom}','{$fmonthto}'):: float,0) :: numeric)*100 ,1),0) as utilization 
						from fac_mvrf_db where {$wc} 
						group by distcode,facode order by distcode,facode) as a 
						group by facode order by facilityname(facode) ) as a order by cat4 ,cat3 ,cat2 ,cat1";
									
				$total_query="Select * from 
						(select distcode,districtname(distcode), 
						count(facode) as totaluc, 
						sum(case when (Access >= 80) and (utilization < 10) then 1 else null end) as cat1, 
						sum(case when (Access >= 80) and (utilization >= 10) then 1 else null end) as cat2, 
						sum(case when (Access < 80) and (utilization < 10) then 1 else null end) as cat3, 
						sum(case when (Access < 80) and (utilization >= 10) then 1 else null end) as cat4 
						from(
						select distcode,facode, 
						COALESCE(round(((sumvaccinevacination(7,fac_mvrf_db.facode,'{$fmonthfrom}','{$fmonthto}')::
						numeric/NULLIF(getmonthlytarget_specificyearrsurvivinginfants(fac_mvrf_db.facode,
						'facility','{$startyear}','{$sm}','{$endyear}','{$em}') :: float,0))*100):: numeric,0),0) as Access, 
						COALESCE(round(((sumvaccinevacination(7,fac_mvrf_db.facode,'{$fmonthfrom}','{$fmonthto}') :: numeric - 
						sumvaccinevacination(9,fac_mvrf_db.facode,'{$fmonthfrom}','{$fmonthto}'):: numeric)/
						NULLIF(sumvaccinevacination(7,fac_mvrf_db.facode,'{$fmonthfrom}','{$fmonthto}'):: float,0) :: 
						numeric)*100 ,1),0) as utilization from fac_mvrf_db where {$wc} 
						group by distcode,facode order by distcode,facode) as a group by distcode 
						order by districtname(distcode)) as a order by cat4 desc,cat3 desc,cat2 desc,cat1 asc";
		
			}
			else	
			{
				$query="Select * from(select uncode,unname(uncode),
						sum(case when (Access >= 80) and (utilization < 10) then 1 else null end) as cat1,
						sum(case when (Access >= 80) and (utilization >= 10) then 1 else null end) as cat2,
						sum(case when (Access < 80) and (utilization < 10) then 1 else null end) as cat3,
						sum(case when (Access < 80) and (utilization >= 10) then 1 else null end) as cat4 
						...
						from fac_mvrf_db where {$wc} group by distcode,uncode order by distcode,uncode) as a group by uncode order by unname(uncode) ) as a order by cat4 ,cat3 ,cat2 ,cat1 ";
					$total_query="Select * from (select distcode,districtname(distcode), count(uncode) as totaluc,
						sum(case when (Access >= 80) and (utilization < 10) then 1 else null end) as cat1,
						sum(case when (Access >= 80) and (utilization >= 10) then 1 else null end) as cat2,
						sum(case when (Access < 80) and (utilization < 10) then 1 else null end) as cat3,
						sum(case when (Access < 80) and (utilization >= 10) then 1 else null end) as cat4 
			...
			$query="Select * from (select distcode,districtname(distcode), count(uncode) as totaluc,
