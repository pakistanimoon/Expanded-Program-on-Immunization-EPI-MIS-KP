...
class Maps_model extends CI_Model {
	...
	function districtWiseMapData($fmonth="2016-01"){
		$query = "
					...
					d2.path 
						FROM districts d1
							LEFT JOIN districts_wise_maps_paths d2 ON d1.distcode=d2.distcode where d1.procode='{$_SESSION['Province']}'";
	...
	function getdistrictWiseMapData($code=NULL,$querySelection,$firstqueryselection=NULL,$lastqueryselection=NULL){
		...
		else
		{
			$query = "
					...
					d2.path 
						FROM districts d1
							LEFT JOIN districts_wise_maps_paths d2 ON d1.distcode=d2.distcode  where d1.procode='{$_SESSION['Province']}' {$lastqueryselection}";
		}
		$query = $this -> db -> query($query);
		return $query -> result();
	}
	...
	public function getVaccineCoverage($data, $selectQuery,$order=NULL){
		...
		if($order){}else{
			...
			$query .= "
						FROM districts
							LEFT JOIN districts_wise_maps_paths d2 ON districts.distcode=d2.distcode where districts.procode='{$_SESSION['Province']}'";
		if($order)
