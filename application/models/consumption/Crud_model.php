...
class Crud_model extends CI_Model {
	...
	/* public function get_existing_items_oldtbl($activity,$whcode,$fmonth)
	{
		$this -> db -> select("epi_item_pack_sizes.item_id as itemid,string_agg(('cr_r'||cr_table_row_numb||'_f6*'||number_of_doses)::text, '+') as openbalformula,epi_items.list_order as rank,epi_items.description itemname,'' as doses,'BB2019' as batch,0 as opening,0 as recdoses,epi_items.in_doses");
		//$this -> db -> select("epi_item_pack_sizes.item_id,string_agg(('cr_r'||cr_table_row_numb||'_f6*'||number_of_doses)::text, '+') as openbalformula");
		$this->db->from("epi_item_pack_sizes");
		$this->db->join("epi_items","epi_items.pk_id = epi_item_pack_sizes.item_id");
        $this -> db -> where('cr_table_row_numb Is Not Null');
        $this -> db -> where('activity_type_id',$activity);
        //$this -> db -> group_by('epi_item_pack_sizes.item_id');
        $this -> db -> group_by('epi_item_pack_sizes.item_id,epi_items.list_order,epi_items.description,epi_items.in_doses');
		$allitems = $this -> db -> get() -> result_array();
		foreach($allitems as $key=>$oneitem){
			$this -> db -> select("$oneitem[openbalformula] as openingbal",FALSE);
			$whereCondition['facode'] = $whcode;
	...
	public function getvaccinesDailyRegister($fmonth,$facode){
		$query="SELECT fac.procode,fac.distcode,fac.tcode,fac.uncode,vac.facode,vac.techniciancode,vac.batch_number as batch,epi_item_pack_sizes.pk_id as itemid,epi_item_pack_sizes.number_of_doses as doses,epi_items.list_order as rank,epi_item_pack_sizes.item_name,epi_items.in_doses,epi_item_pack_sizes.item_category_id,vac.fmonth,sum(vac.vaccinated) as vaccinated,sum(vac.used_vials) as used_vials,sum(vac.used_doses) as used_doses,sum(vac.unused_vials) as unused_vials,sum(vac.unused_doses) as unused_doses,0 as opening,0 as recdoses
		FROM vaccine_vials_daily_record vac join facilities fac on vac.facode=fac.facode
		LEFT OUTER JOIN epi_item_pack_sizes on vac.item_id=epi_item_pack_sizes.pk_id  
		LEFT OUTER JOIN epi_items on epi_item_pack_sizes.item_id = epi_items.pk_id 
		where vac.fmonth='$fmonth' and vac.facode='$facode' 
		group by epi_item_pack_sizes.pk_id,vac.fmonth,vac.batch_number,fac.procode,fac.distcode,fac.tcode,fac.uncode,vac.facode,vac.techniciancode,epi_items.list_order,epi_item_pack_sizes.number_of_doses,epi_item_pack_sizes.item_name,epi_items.in_doses,epi_item_pack_sizes.item_category_id";
