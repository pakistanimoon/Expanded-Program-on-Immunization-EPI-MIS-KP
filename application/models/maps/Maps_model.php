...
// new code upload by zeeshan start
	...
	{
		$query = "SELECT fweek, count(*) as cnt FROM corona_case_investigation_form_db group by fweek order by fweek asc";
		$query = $this->db->query($query);
		return $query->result();
	}
	
	function covid19Summary($distcode=NULL){
		$procode = $this -> session -> Province;
		$this -> db -> select("
			sum(case when case_epi_no IS NOT NULL then 1 else 0 end) as suspected,
			sum(case when test_result = 'Positive' AND case_epi_no IS NOT NULL then 1 else 0 end) as confirmed, 
			sum(case when test_result = 'Negative' AND case_epi_no IS NOT NULL then 1 else 0 end) as negative, 
			sum(case when gender = 1 AND case_epi_no IS NOT NULL then 1 else 0 end) as male, 
			sum(case when gender = 0 AND case_epi_no IS NOT NULL then 1 else 0 end) as female, 
			sum(case when (test_result <> 'Positive' AND case_epi_no IS NOT NULL AND test_result <> 'Negative') OR (test_result IS NULL AND case_epi_no IS NOT NULL) then 1 else 0 end) as \"resultawaited\", 
			sum(case when test_result = 'Positive' AND case_epi_no IS NOT NULL then 1 else 0 end) - sum(case when test_result = 'Positive' AND outcome = 'Recovered' AND case_epi_no IS NOT NULL then 1 else 0 end) - sum(case when outcome = 'Death' AND case_epi_no IS NOT NULL then 1 else 0 end) as activecases,
			sum(case when test_result = 'Positive' AND outcome = 'Recovered' AND case_epi_no IS NOT NULL then 1 else 0 end) as recovered, 
		...
		$this -> db -> get('corona_case_investigation_form_db') -> row();
		$innerQuery = $this -> db -> last_query();
		$this -> db -> select("*, (male//suspected)*100 perc_male, (female//suspected)*100 perc_female, (confirmed//suspected)*100 perc_confirmed, (negative//suspected)*100 perc_negative, (resultawaited//suspected)*100 perc_resultawaited, (activecases//suspected)*100 perc_activecases, (recovered//suspected)*100 perc_recovered, (deaths//suspected)*100 perc_deaths");
		$this -> db -> from("($innerQuery) as a");
		return $this -> db -> get() -> row();
	}
	
	function districtWiseCovid19Summary($distcode=NULL){
		$procode = $this -> session -> Province;
		if($distcode){
			$this -> db -> select("
				...
				sum(case when gender = 1 AND case_epi_no IS NOT NULL then 1 else 0 end) as male, 
				sum(case when gender = 0 AND case_epi_no IS NOT NULL then 1 else 0 end) as female, 
				sum(case when (test_result <> 'Positive' AND test_result <> 'Negative' AND case_epi_no IS NOT NULL) OR (test_result IS NULL AND case_epi_no IS NOT NULL) then 1 else 0 end) as \"resultawaited\", 
				sum(case when test_result = 'Positive' AND case_epi_no IS NOT NULL then 1 else 0 end) - sum(case when test_result = 'Positive' AND outcome = 'Recovered' AND case_epi_no IS NOT NULL then 1 else 0 end) - sum(case when outcome = 'Death' AND case_epi_no IS NOT NULL then 1 else 0 end) as activecases,
				sum(case when test_result = 'Positive' AND outcome = 'Recovered' then 1 else 0 end) as recovered, 
		...
		$innerQuery = $this -> db -> last_query();
		if($distcode){
			$this -> db -> select("unname(uc.uncode) as districtname, uc.uncode as districtcode, a.*, (male//suspected)*100 perc_male, (female//suspected)*100 perc_female, (confirmed//suspected)*100 perc_confirmed, (negative//suspected)*100 perc_negative, (resultawaited//suspected)*100 perc_resultawaited, (activecases//suspected)*100 perc_activecases, (recovered//suspected)*100 perc_recovered, (deaths//suspected)*100 perc_deaths");
			$this -> db -> from("($innerQuery) as a");
			$this -> db -> join('unioncouncil uc','uc.uncode=a.uncode','right');
		}else{
			$this -> db -> select("dist.district as districtname, dist.distcode as districtcode, a.*, (male//suspected)*100 perc_male, (female//suspected)*100 perc_female, (confirmed//suspected)*100 perc_confirmed, (negative//suspected)*100 perc_negative, (resultawaited//suspected)*100 perc_resultawaited, (activecases//suspected)*100 perc_activecases, (recovered//suspected)*100 perc_recovered, (deaths//suspected)*100 perc_deaths");
			$this -> db -> from("($innerQuery) as a");
	...
	function getCovidWeeklySuspectedCasesDetail($distcode=NULL)
	{
		$procode = $this -> session -> Province;
	...
	function getCovidWeeklyConfirmedCasesDetail($distcode=NULL)
	{
		$procode = $this -> session -> Province;
		$this -> db -> select("fweek, count(*) as cnt");
		$this -> db -> from('corona_case_investigation_form_db');
		$this -> db -> where(array('procode' => $procode,'test_result' => 'Positive'));
		$this -> db -> where('case_epi_no IS NOT NULL');
		if($distcode)
			$this -> db -> where('distcode',$distcode);
		$this -> db -> group_by('fweek');
	...
	function getCovidWeeklyDeathsCasesDetail($distcode=NULL)
	{
		$procode = $this -> session -> Province;
		$this -> db -> select("fweek, count(*) as cnt");
		$this -> db -> from('corona_case_investigation_form_db');
	...
	function getCovidWeeklyRecoveredCasesDetail($distcode=NULL)
	{
		$procode = $this -> session -> Province;
		$this -> db -> select("fweek, count(*) as cnt");
		$this -> db -> from('corona_case_investigation_form_db');
		$this -> db -> where(array('procode' => $procode, 'outcome' => 'Recovered', 'test_result' => 'Positive'));
		$this -> db -> where('case_epi_no IS NOT NULL');
		if($distcode)
			$this -> db -> where('distcode',$distcode);
		$this -> db -> group_by('fweek');
	...
	function getCovidWeeklyMaleCasesDetail($distcode=NULL)
	{
		$procode = $this -> session -> Province;
		$this -> db -> select("fweek, count(*) as cnt");
		$this -> db -> from('corona_case_investigation_form_db');
		$this -> db -> where(array('procode' => $procode, 'gender' => 1));
		$this -> db -> where('case_epi_no IS NOT NULL');
		if($distcode)
			$this -> db -> where('distcode',$distcode);
		$this -> db -> group_by('fweek');
	...
	function getCovidWeeklyFemaleCasesDetail($distcode=NULL)
	{
		$procode = $this -> session -> Province;
		$this -> db -> select("fweek, count(*) as cnt");
		$this -> db -> from('corona_case_investigation_form_db');
		$this -> db -> where(array('procode' => $procode, 'gender' => 0));
		$this -> db -> where('case_epi_no IS NOT NULL');
		if($distcode)
			$this -> db -> where('distcode',$distcode);
		$this -> db -> group_by('fweek');
	...
	function getActiveCasesSeriesData($distcode=NULL){
		$procode = $this -> session -> Province;
		if($distcode){
			$this -> db -> select("
				d2.uncode as code,unname(d2.uncode) as name, d2.path,
				sum(case when test_result = 'Positive' AND case_epi_no IS NOT NULL then 1 else 0 end) - sum(case when test_result = 'Positive' AND outcome = 'Recovered' AND case_epi_no IS NOT NULL then 1 else 0 end) - sum(case when outcome = 'Death' AND case_epi_no IS NOT NULL then 1 else 0 end) as value");
		}else{
			$this -> db -> select("
				d2.distcode as code,districtname(d2.distcode) as name, d2.path,
				sum(case when test_result = 'Positive' AND case_epi_no IS NOT NULL then 1 else 0 end) - sum(case when test_result = 'Positive' AND outcome = 'Recovered' AND case_epi_no IS NOT NULL then 1 else 0 end) - sum(case when outcome = 'Death' AND case_epi_no IS NOT NULL then 1 else 0 end) as value");
		}
		$this -> db -> from('corona_case_investigation_form_db corona');
		if($distcode)
			$this -> db -> join('uc_wise_maps_paths d2','corona.uncode=d2.uncode','right');
		else
			$this -> db -> join('districts_wise_maps_paths d2','corona.distcode=d2.distcode','right');
