									...
									<tr>	
										...
										<!--<td>
											<select class="form-control text-center hard_to_reach" name="hard_to_reach[<?php echo $key+1; ?>]" required="required">
												<option value="<?php echo $val['f2_hard_to_reach'] ?>"><?php echo $val['f2_hard_to_reach'] ?></option>
											</select>
										</td>-->	
										<td><input type="text" name="hard_to_reach_population[<?php echo $key+1; ?>]" class="form-control text-center numberclass asp calculation hard_to_reach_population"></td>
									</tr>	
								<?php } ?>								
							</tbody>
						</table>
...
//				var year = $('#fyear').val();
			 ...
			 /////////////////////
				...
				if(session_type !== 'Mobile'){
					if(session_type == 'Fixed'){
						...
						var spm = Math.ceil(parseFloat((num_sess)/80));
						if( ! isNaN(spm)){
					...
					if(session_type == 'Outreach'){
						var num_sess = parseInt($(this).closest('tr').find(".injections_per_month").val());
						var spm = Math.ceil(parseFloat((num_sess)/40));
						if( ! isNaN(spm)){
						   $(this).closest('tr').find(".sessions_per_month").val(spm);
						}
						else{
					...
					var num_sess = parseInt($(this).closest('tr').find(".injections_per_month").val());
					var spm = Math.ceil(parseFloat((num_sess)/1));
					if( ! isNaN(spm)){
					   $(this).closest('tr').find(".sessions_per_month").val(spm);
					}
					else{
			...
			$(document).on('keyup','.calculation',function(e){
				...
				if(hard_to_reach !== 'Yes'){
					...
					//$(this).closest('tr').find(".hard_to_reach_population").css("background-color","#eee");
