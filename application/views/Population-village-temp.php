...
<?php //echo '<pre>'; print_r($data);exit; ?>
    ...
    <div class="panel panel-primary">
      ...
      <div class="panel-heading"> Villages Population Form
        </div>
		...
		<div class="panel-body">
          ...
          <tr>
                ...
                <th class="text-center Heading">Village Name</th>
...
<?php $index=1; ?>
          ...
          <?php foreach ($data as $i => $value) { ?>
            <tr>
               ...
               <td class='text-left'>
			   <input type='text' name='villagename[]' value="<?php echo $value->village; ?>" readonly class='form-control'>
			   <input type='hidden' name='village[]' value="<?php echo $value->vcode; ?>" readonly class='form-control'>
			   </td>
               <td class='text-left' ><input name='unname[]' readonly value="<?php echo $value->unname; ?>" class='form-control text-left'></td>
			   
			    <!---  <td class='text-left' ><input name='facodename[]' readonly value="<php echo $value->facode; ?>" class='form-control text-left'></td>--->
			   
			 <!---Start   <td class="text-align-last: center;"> 
									
			   <php 
					$str = $value->facode;
					if(strlen($str) > 0 ){
					echo $value->facode;					
					// echo FacilityNameByuncode($value->uncode); 
					 }	else	{
					echo 'e';	echo $value->facode;					 
					//echo FacilityNameByuncode($value->uncode); 
					} ?>
			 ...
			 <!--- <php
			  ...
			  <php//echo 'if'; ?>
					<select name="facode[]" id="facode[]">
					<php
					  //$isSelected = (isset($selected) && $selected==$value["facode"])?'selected="selected"':'';
					  echo FacilityNameByuncode($value->uncode);
					  ?>
					</select>
						</td>				
			   ...
			   <td class="text-align-last: center;"> 
			     <php//echo 'Else'; ?>
					<select name="facode[]" id="facode[]">
							<option value="0" selected> -- Select Facility -- </option>
									<phpecho FacilityNameByuncode($value->uncode); ?>
					</select> 
						</td>	
				<php} ?> --->
				 <td class="text-align-last: center;"> 	
				<select name="facode[]" id="facode[]">		
					...
					<?php 
						if(isset($value) && $value->facode > 0) { 
								/* echo 'if';
								echo $value->uncode; */
							echo getFLCF_options(false, $value->facode, $value->uncode); 
						 } else {
								/* echo 'else';
								echo $value->uncode; */
							echo getFLCF_options(false, $value->facode, $value->uncode); 
