...
<style>
		...
		<?php
			...
			if(isset($_REQUEST['reportType']) AND $_REQUEST['reportType'] == 0 AND  ! isset($_REQUEST['drilldownType'])){
				$reportType = 'singledate_technicianwise';
			}
		?>
	<div id="month" align="center">
		...
		<?php
			$nmonth = Date("Y-m", strtotime($data['fmonth'] . " next month")); 
			$premonth = Date("Y-m", strtotime($data['fmonth'] . " previous month"));
			$month = Date("Y-m", strtotime($data['fmonth']));
		?>
		<a href="<?php echo base_url(); ?>childs/DailyRegisterChildReport/monthly_report/?
				reportType=<?php echo '0'; ?>&
				...
				techniciancode=<?php if (isset($techniciancode)){echo $techniciancode;}?>&
				drilldownType=<?php echo (isset($drilldownType))?$drilldownType:''; ?>" class="previous round"><strong>&lt;&lt; </strong> 
		</a>
		<strong>
				<?php echo $month;?>
		</strong>
		<a href="<?php echo base_url(); ?>childs/DailyRegisterChildReport/monthly_report/?
				...
				facode=<?php if (isset($facode)){echo $facode;}?>&
				techniciancode=<?php if (isset($techniciancode)){echo $techniciancode;}?>&
				drilldownType=<?php echo (isset($drilldownType))?$drilldownType:''; ?>" class="next round"> <strong>&gt;&gt; </strong>
		</a>
	</div>
		<div id="parent">
			<table id="fixTable" class="table table-bordered table-hover table-striped">
				<thead>
					<tr>
					<th class="Heading text-center" style="background: #008d4c; color: white; width: 200px; border: 1px solid black" rowspan="3">Month Date</th>	
					</tr>
					<tr> 					
						...
						<th class="Heading text-center" style="background: #008d4c; color: white; width: 200px; border: 1px solid black; rowspan=1">Children Vaccinated</th>
					</tr>
				</thead>
				<tbody>
					<?php 
					...
					<tr class="DrillDownRow" style="cursor: pointer;">
						<td style='text-align:center; border: 1px solid black' class='text-center'><?php echo $val['monthdate']; ?></td>
...
$(document).on('click',".DrillDownRow", function(){
	var monthdate = $(this).find("td:first").text();
	...
	var techniciancode = '<?php echo (isset($techniciancode))?$techniciancode:''; ?>';
	var reportType = '<?php echo $reportType; ?>';
	url = "<?php echo base_url();?>childs/DailyRegisterChildReport/monthly_report?reportType="+reportType+"&monthdate="+monthdate+"&distcode="+distcode+"&tcode="+tcode+"&uncode="+uncode+"&facode="+facode+"&techniciancode="+techniciancode+"&drilldownType=techniciancode";
	if(reportType == 0)
		url = "<?php echo base_url();?>childs/DailyRegisterChildReport/monthly_report?monthdate="+monthdate+"&distcode="+distcode+"&tcode="+tcode+"&uncode="+uncode+"&facode="+facode+"&techniciancode="+techniciancode+"&drilldownType=techniciancode";
