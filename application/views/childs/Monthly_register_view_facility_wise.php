...
<style>
	...
	<div id="month" align="center">
		...
		<a href="<?php echo base_url(); ?>/childs/DailyRegisterChildReport/monthly_report/?
				...
				facode=<?php if (isset($facode)){echo $facode;}?>" class="previous round"><strong>&lt;&lt; </strong> 
		</a>
		<strong>
				<?php echo $month;?>
		</strong>
		<a href="<?php echo base_url(); ?>/childs/DailyRegisterChildReport/monthly_report/?
				...
				tcode=<?php if (isset($tcode)){echo $tcode;}?>&
				uncode=<?php if (isset($uncode)){echo $uncode;}?>&
				facode=<?php if (isset($facode)){echo $facode;}?>" class="next round"> <strong>&gt;&gt; </strong>
		</a>
	</div>
		<div id="parent">
			<table id="fixTable" class="table table-bordered table-hover table-striped">
				<thead>
					<tr>
						...
						<th class="Heading text-center" style="background: #008d4c; color: white; width: 200px; border: 1px solid black" rowspan="3">Facility Name</th>	
					</tr>
					<tr> 					
				...
				<tbody>
					...
					<tr class="DrillDownRow" style="cursor: pointer;">
						<td style='text-align:center; border: 1px solid black' class='text-center'><?php echo $val['reg_facode']; ?></td>
						<td style='text-align:center; border: 1px solid black' class='text-center'><?php echo $val['facilityname']; ?></td>
...
$(document).on('click',".DrillDownRow", function(){
	var facode = $(this).find("td:first").text();
	...
	var reportType = '0';
	var drilldownType='flcf';
	url = "<?php echo base_url();?>childs/DailyRegisterChildReport/monthly_report/?reportType="+reportType+"&monthfrom="+monthfrom+"&distcode="+distcode+"&tcode="+tcode+"&uncode="+uncode+"&facode="+facode+"&drilldownType="+drilldownType;
