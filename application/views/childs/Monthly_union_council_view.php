...
<style>
	...
	<a href="<?php echo base_url(); ?>/childs/DailyRegisterChildReport/monthly_report/?
				...
				distcode=<?php if (isset($distcode)){echo $distcode;}?>&
				tcode=<?php if (isset($tcode)){echo $tcode;}?>&
				uncode=<?php if (isset($uncode)){echo $uncode;}?>"class="next round"> <strong>&gt;&gt; </strong>
	</a>
</div>
		<div id="parent">
		...
		<thead>
			<tr> 
			<th class="Heading text-center" style="background: #008d4c; color: white; width: 200px; border: 1px solid black; rowspan=1">UNCODE</th>
			<th class="Heading text-center" style="background: #008d4c; color: white; width: 200px; border: 1px solid black; rowspan=1">UC</th>
		...
		<tbody>
			...
			<!--<td style='text-align:center; border: 1px solid black' class='text-center'><?php echo CrossProvince_UCName($val['uncode']); ?></td>-->
...
$(document).on('click',".DrillDownRow", function(){
	var uncode = $(this).find("td:first").text();
	...
	var reportType = '0';
	var drilldownType='uc';
	url = "<?php echo base_url();?>childs/DailyRegisterChildReport/monthly_report/?reportType="+reportType+"&monthfrom="+monthfrom+"&distcode="+distcode+"&tcode="+tcode+"&uncode="+uncode+"&drilldownType="+drilldownType;
