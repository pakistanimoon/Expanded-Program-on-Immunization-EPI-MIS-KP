...
<div class="row">
  <?php if($this -> session -> flashdata('message')){  ?><div class="alert alert-success text-center" role="alert"><strong><?php echo $this -> session -> flashdata('message'); ?></strong></div> <?php } ?>
   ...
   <ol class="breadcrumb">
      ...
      <div class="panel-heading"> List of Refrigerator Questionnaires
        </div>
              ...
              <tr>
                ...
                <?php if ($_SESSION['UserLevel']=='3' && $utype=='DEO' ){?>
                  <th class="text-center Heading">
                     <a href="<?php echo base_url(); ?>Refrigerator-Questionnaire/Add" data-toggle="tooltip" title="Add New Refrigerator Questionnaire">
  ...
  foreach($results as $row){ 
    ...
    <tr class="DrilledDown"> 
      ...
      <td class="text-center">
        <a data-original-title="View" href="<?php echo base_url(); ?>Refrigerator-Questionnaire/View/<?php echo $row['id']?>" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-search"></i></a>
        <a data-original-title="Edit" href="<?php echo base_url(); ?>Refrigerator-Questionnaire/Edit/<?php echo $row['id']?>" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
      ...
      $('#tbody').html('<h1><td colspan="10" class="text-center" ><img src="<?php echo base_url(); ?>includes/images/ajax-loader_blue.gif"> loading...</td></h1>');
        ...
        $.ajax({
         ...
         // data: $('#filter-form').serialize(),
          dataType:"json",
          url: "<?php echo base_url(); ?>Ajax_calls/refrigerator_questionnaire_filter/"+page+"/"+tcode+"/"+facode+"/"+fatype+"/"+distcode,
...
$('.DrilledDown').css('cursor','pointer');
    $(document).on('click',".DrilledDown", function(){
       var supervisorcode = $(this).find("td:nth-child(2)").text();
        var url = '';
        url = "<?php echo base_url();?>Refrigerator-Questionnaire/View/"+supervisorcode;
