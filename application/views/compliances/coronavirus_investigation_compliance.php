	...
	<?php if(!$this->input->post('export_excel')){ ?>
		$(document).on('click','.mrClicked', function(){
			...
			if(code.toString().length == 6){
				//url = "<?php echo base_url();?>Measles-CIF/View/"+code+"/"+id;
				url = "<?php echo base_url();?>Linelists/Surveillance/"+code+"/"+year+"/"+week+"/Measles_other";
			}
		...
		$(document).on('click','.clickedReport', function(){
			...
			if(code.toString().length == 3){
				url = "<?php echo base_url();?>Compliances/Coronavirus_Compliance/"+code+"/"+year+"/"+from_week+"/"+to_week;
			}
			var win = window.open(url,'_self');
			if(win){
				//Browser has allowed it to be opened
