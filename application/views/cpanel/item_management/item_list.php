            ...
            <table class="table table-bordered table-hover table-striped footable table-vcenter" data-filter="#filter" data-filter-text-only="true" id="itemList">
               <thead>
                  <tr class="info">
                    ...
                    <th class="text-center Heading">Pack Sizes</th>
                     <th class="text-center Heading">Category</th>
                     <th class="text-center Heading">
						<button id="add_button" class="submit custom-add-btn btn-sm" onclick="location.href='<?php echo base_url(); ?>Item_management/item_add'">
               ...
               <tbody id="tbody"> 
                  ...
                  <tr id="row" class="DrilledDown">
                     ...
                     <td class="text-left"><?php echo $row['description']; ?></td>
                      <td class="text-left"><?php echo $row['number_of_doses']; ?></td>
                     <td class="text-left"><?php echo $row['item_category_id']; ?></td>
                     <td class="text-center">
                        <a data-original-title="Edit" href="<?php echo base_url(); ?>Item_management/item_edit/<?php echo $row['pk_id']; ?>" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
...
<script type="text/javascript">
   $(function () {
	...
	function del_item(obj){
		//alert(obj); exit();
		var myurl = '<?php echo base_url(); ?>Item_management/item_del/'+obj;		
   ...
   $(document).ready(function() {
	   ...
	   var columns = [
			...
			{ data: "description" },
			{ data: "number_of_doses" },
			{ data: "item_category_id" },
			{ data: "pk_id",
				orderable: false,
				render : function(data, type, row) 
					{				
					 return '<a data-original-title="Edit" href="<?php echo base_url(); ?>Item_management/item_edit/'+data+'" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a> <a data-original-title="Delete" href="javascript:void(0);" onclick="javascript:del_item(\''+data+'\')" data-toggle="tooltip" title="" class="btn btn-xs btn-danger" ><i class="fa fa-times"></i></a>'
					}
			},
		]; 
		var table = $('#itemList').DataTable(
		{
			...
			"ajax": {
				url : "<?php echo base_url(); ?>Ajax_hr_management/item_list_search",
				type : 'GET'
			},
			"columns": columns,
			dom: 'lrtips',
