				...
				<form method="post" id="filter-form">
					...
					<div class="row" style="margin-top:5px;">   
						<div class="form-group">
							...
							<label class="col-xs-2 col-xs-offset-1 control-label lbl-setting" for="tcode">Tehsil:</label>
							<div class="col-xs-3">
								<select id="tcode" name="tcode" onchange="search();" class="filter-status  form-control">
									<option value="0"></option>
									<?php foreach($resulttehsil as $row) { ?>
					...
					<div class="row" style="margin-top:5px;">   
						<div class="form-group">							
							...
							<?php } ?>  -->
							<?php if($UserLevel == 2 || $UserLevel == 3 ) { ?>
								<label class="col-xs-2 col-xs-offset-1 control-label lbl-setting"  for = "facode">Health Facility:</label>
								<div class="col-xs-3">
									<select id="facode2" name="facode" onchange="search();" class="filter-status form-control">
										<option value="0"></option>
					...
					<thead>
						<tr>
							...
							<?php if (($_SESSION['UserLevel']=='3') && ($_SESSION['utype']=='DEO') ){?>
								<a href="<?php echo base_url(); ?>AEFI-CIF/Add">
					...
					<tbody id="tbody"><?php
						...
						foreach($result as $row){
							...
							<tr>
								...
								<td class="text-center">
									<?php
										if($row['vacc_name']>=1 && $row['vacc_name']<=100000){
											echo getVaccineName($row['vacc_name']);
										}
										else{
											echo $row['vacc_name'];
										}
...
<script type="text/javascript">
	...
	function delete_report(rpt){
        ...
        if(response==true && id!="" && year!="") {
            window.location.href="<?php echo base_url(); ?>Data_entry/aefi_delete/"+id
        }
        else {
            //do nothing
        }
