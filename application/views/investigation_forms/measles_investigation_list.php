...
<div class="row">
      ...
      <div class="panel-heading"> List of Measles Investigation Form</div>
						...
						<div class="col-xs-3">
							<select id="distcode" name="distcode" class="filter-status form-control">
								...
								<?php $distcode = $this -> session -> District; { ?>
									...
									<option style="background: #8FEBAD;" value="<?php echo 'by_dist'; ?>" >Approved cases cross notified by <?php echo get_District_Name($this -> session -> District); ?></option>
									<option style="background: #EBD38F;" value="<?php echo 'from_dist'; ?>" >Approved cases cross notified to <?php echo get_District_Name($this -> session -> District); ?></option>
									<option style="background: rgba(219, 37, 37, 0.5);" value="<?php echo 'pending_dist'; ?>" >Pending cases within <?php echo get_District_Name($this -> session -> District); ?> and KPK</option>
									<option style="background: #33ACFF;" value="<?php echo 'other_prov_dist'; ?>" >Pending cross province cases</option>								
								<?php } ?>
							</select>
						</div> 
					</div>
				...
				<thead>
					<tr>
						...
						<th class="text-center Heading">
							<a href="<?php echo base_url(); ?>Measles_investigation/measles_investigation" data-toggle="tooltip" title="Add New Case Investigation Form">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(document).on("click",".paginateMe",  function (e){
			...
			$.ajax({
				...
				dataType:"json",
				url: "<?php echo base_url(); ?>Ajax_red_rec/measles_investigation_filter?page="+page,
	...
	$('.filter-status').on('change' , function (){
      ...
      $.ajax({
			type: "GET",
			data: $('#filter-form').serialize(),
			url: "<?php echo base_url(); ?>Ajax_red_rec/measles_investigation_filter",
			dataType: "json",
			success: function(result){
				$('#tbody').html('');
				if(result != null){
	...
	function delete_report(rpt){
        ...
        {
            window.location.href="<?php echo base_url(); ?>Measles_investigation/measles_investigation_delete/"+id+"/"+year+"";
        }
        else
        {
            //do nothing
