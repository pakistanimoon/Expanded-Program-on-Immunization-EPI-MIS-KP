...
<div class="container bodycontainer">
   ...
   <div class="panel-heading"> List of NNT Investigation Form</div>
   <div class="panel-body">
		<form method="post" id="filter-form">
         ...
         <div class="row" style="margin-top:5px;">   
            <div class="form-group">
               <label class="col-xs-2 col-xs-offset-1 control-label lbl-setting" >Investigated By:</label>
               <div class="col-xs-3">
                  <select id="investigated_by" name="investigated_by" class="filter-status form-control">
                     <option value=""></option>
                     <?php foreach($resultInvest_by as $row){?>
                     <option value="<?php echo $row['investigated_by'];?>" ><?php echo $row['investigated_by'];?></option>
                     <?php  } ?>
                  </select>
               </div>          
               <label class="col-xs-2 control-label lbl-setting"  for = "facode" >UCs:</label>
               <div class="col-xs-3">
...
<table class="table table-bordered table-hover table-striped footable table-vcenter tbl-listing" data-filter="#filter" data-filter-text-only="true">
            <thead>
              <tr>
				...
				<th class="text-center Heading">Investigated By</th>
				<th class="text-center Heading">Year-Week</th>
        <th class="text-center Heading">Cross Notified(District)</th>
				<th class="text-center Heading">Date of Notification</th>
...
?>
        <tr style="<?php echo $color; ?>">
          <td class="text-center"><?php echo $i; ?></td>
					<td class="text-left"><?php echo $row['full_mother_name']; ?></td>
         ...
         <td class="text-left"><?php echo isset($row['uncode']) && $row['uncode']!= NULL && $row['uncode']!= ''?CrossProvince_UCName($row['uncode']): ''; ?></td>
					<td class="text-left"><?php echo $row['investigated_by']; ?></td>
         ...
         <?php } ?>
					...
					<td class="text-center"><?php if(($row['date_notification'])!=''){ echo date("d-M-Y",strtotime($row['date_notification']));} ?></td>
					<td class="text-center"><?php if(($row['date_investigation'])!=''){ echo date("d-M-Y",strtotime($row['date_investigation']));} ?></td>
...
<script type="text/javascript">
   ...
   $('.filter-status').on('change' , function (){
      ...
      url: "<?php echo base_url(); ?>Ajax_red_rec/nnt_investigation_filter",
      dataType: "json",
   ...
   function crossNotifiedfweek(){
      ...
      $.ajax({
         type: "GET",
         data: $('#filter-form').serialize(),
         url: "<?php echo base_url(); ?>Ajax_red_rec/cross_notified_nnt_investigation_filter",
         dataType: "json",
         success: function(result){
            $('#tbody').html('');
            if(result != null){
