...
<div class="row">
    <div class="panel panel-primary">
     ...
     <ol class="breadcrumb">
      ...
      <div class="panel-heading"> List of Line List for NNT Cases
        </div>
...
<table class="table table-bordered table-hover table-striped footable table-vcenter" data-filter="#filter" data-filter-text-only="true">
            <thead>
              <tr class="success">
                ...
                <th class="text-center Heading">Reported From</th>
                 <th class="text-center Heading">
                  <a href="<?php echo base_url(); ?>Linelists/nnt_linelist" data-toggle="tooltip" title="Add New NNT LineList">
...
?>
                           ...
                           <tr>
                              ...
                              <td class="text-center"><?php echo $row['reported_from']; ?></td>
                              <td class="text-center">
                                <a href="<?php echo base_url(); ?>Linelists/nnt_linelist_view/<?php echo $row['distcode']; ?>/<?php echo $row['linelist_group']; ?>" data-toggle="tooltip" title="View" class="btn btn-xs btn-default"><i class="fa fa-search"></i></a>
                              	<a href="<?php echo base_url(); ?>Linelists/nnt_linelist_edit/<?php echo $row['distcode']; ?>/<?php echo $row['linelist_group']; ?>" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
                              </td>	
                           </tr>
    ...
    $(document).on("click",".paginateMe",  function (e){
      ...
      $('#tbody').html('<h1><td colspan="10" class="text-center" ><img src="<?php echo base_url(); ?>includes/images/ajax-loader_blue.gif"> loading...</td></h1>');
        ...
        $.ajax({
          ...
          dataType:"json",
          url: "<?php echo base_url(); ?>Ajax_calls/nnt_linelist_filter?page="+page,
  ...
  $('.filter-status').on('change' , function (){
    ...
    $('#tbody').html('<h1><td colspan="10" class="text-center" ><img src="<?php echo base_url(); ?>includes/images/ajax-loader_blue.gif"> loading...</td></h1>');
        ...
        $.ajax({
          type: "GET",
          data: $('#filter-form').serialize(),
          url: "<?php echo base_url(); ?>Ajax_calls/nnt_linelist_filter",
          dataType: "json",
          success: function(result){
            console.log(result);
            $('#tbody').html('');
