...
<div class="row">
 <div class="panel panel-primary">
   <div class="panel-heading"> Pneumonia Outbreak Investigation Line List of Suspected Cases</div>
     <div class="panel-body">
       <form class="form-horizontal" method="post" action="<?php echo base_url(); ?>Linelists/pneumonia_linelist_save" >
        <?php if(isset($pneumoniaLineListResult)){ ?>
        	<input type="hidden" name="edit" id="edit" value="edit" />
       		<input type="hidden" name="groupId" id="groupId" value="<?php echo $pneumoniaLineListResult[0]['linelist_group']; ?>" />
        <?php } ?>
        <table class="table table-bordered   table-striped table-hover  mytable">
          <tr>
            ...
            <td><select id="tcode" name="tcode" class="form-control">
            	<?php if(isset($pneumoniaLineListResult)){ 
            		getTehsils_options(false,$pneumoniaLineListResult[0]['tcode']);
            	}else{ ?>
          ...
          <tr>
            ...
            <td><select id="uncode" name="uncode" class="form-control">
            	<?php if(isset($pneumoniaLineListResult)){
            		getUCs_options(false,$pneumoniaLineListResult[0]['uncode']);
            	}else{ ?>
            	<?php getUCs_options(false); } ?>
            </select></td>
           	<td><label style="margin-top: 7px;">Village/Mahalla</label></td>
            <td><input id="village_mahalla" name="village_mahalla" class="form-control" value="<?php if(isset($pneumoniaLineListResult)){ echo $pneumoniaLineListResult[0]['village_mahalla']; } ?>" /></td>
            <td><label style="margin-top: 7px;">Investigation done by</label></td>
            <td><input class="form-control" name="investigation_by" id="investigation_by" value="<?php if(isset($pneumoniaLineListResult)){ echo $pneumoniaLineListResult[0]['investigation_by']; } ?>" type="text"></td>
          </tr>
          <tr>
            <td><label style="margin-top: 7px;">Date of Investigation</label></td>
            <td><input class="dp form-control" name="date_investigation" id="date_investigation" value="<?php if(isset($pneumoniaLineListResult)){ echo date('d-m-Y',strtotime($pneumoniaLineListResult[0]['date_investigation'])); } ?>" type="text"></td>
          </tr>
      </table>
          	...
          	<?php if(isset($pneumoniaLineListResult)){ 
            	foreach($pneumoniaLineListResult as $key => $row){ ?>
            		<tr>
		              ...
		              <td><input class="form-control expand" name="fname_father[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo $row['fname_father']; } ?>" type="text"></td>
		              <td><input class="form-control expand epidnumber" name="case_epi_no[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo $row['case_epi_no']; } ?>" id="case_epi_no" onkeyup="appendNewRow(this.id);" type="text"></td>
		              <td><input class="form-control expand numberclass" name="age_in_months[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo $row['age_in_months']; } ?>" type="text"></td>
		              <td><select name="gender[<?php echo $key; ?>]" class="form-control">
					  <option value="">Select</option>
		              <option <?php if(isset($pneumoniaLineListResult) && ($row['gender'] == 'Male')){ echo 'selected="selected"'; } ?> value="Male">Male</option>
		              <option <?php if(isset($pneumoniaLineListResult) && ($row['gender'] == 'Female')){ echo 'selected="selected"'; } ?> value="Female">Female</option>
		              </select></td>
		              <td><input class="form-control expand" name="child_address[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo $row['child_address']; } ?>" type="text"></td>
		              <td><input class="form-control expand numberclass" name="vacc_dose_no[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo $row['vacc_dose_no']; } ?>" type="text"></td>
		              <td><input class="dp form-control" name="date_last_dose[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo date('d-m-Y',strtotime($row['date_last_dose'])); } ?>" type="text"></td>
		              <td><input class="dp form-control" name="date_rash_onset[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo date('d-m-Y',strtotime($row['date_rash_onset'])); } ?>" type="text"></td>
		              <td><input class="dp form-control" name="date_collection_blood[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo date('d-m-Y',strtotime($row['date_collection_blood'])); } ?>" type="text"></td>
		              <td><input class="dp form-control" name="date_collection_throat[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo date('d-m-Y',strtotime($row['date_collection_throat'])); } ?>" type="text"></td>
		              <td><input class="dp form-control" name="date_follow_up[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo date('d-m-Y',strtotime($row['date_follow_up'])); } ?>" type="text"></td>
		              <td><input class="form-control" name="complication[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo $row['complication']; } ?>" type="text"></td>
		              <td><input class="dp form-control" name="date_death[<?php echo $key; ?>]" value="<?php if(isset($pneumoniaLineListResult)){ echo date('d-m-Y',strtotime($row['date_death'])); } ?>" type="text"></td>
		            </tr>
            	<?php } }else{ ?>
            <tr>
