	<div class="container">
		<div class="row">
			<div class="panel panel-primary">
				...
				<div class="panel-heading" style="font-size:17px; border-color:white !important;">Using the RED Strategy for Problem Solving List</div>
					...
					<form method="post" id="filter-form">
						<div class="row" style="width:100%; padding:4px 17px">
							...
							<div class="col-md-3">
								<?php
									...
									$query="SELECT distinct tcode, tehsilname(tcode) as tehsil from red_strategy_db where distcode='{$distcode}' order by tehsil ASC";
							...
							<div class="col-md-3">
								<?php
									$distcode = $this-> session-> District; 
									$query="SELECT distinct uncode, unname(uncode) as uc_name from red_strategy_db where distcode='{$distcode}' order by uc_name ASC";
									$result = $this->db->query($query)->result_array();
								?>
						...
						<div class="row" style="width:100%; padding:4px 17px">					
							...
							<div class="col-md-3">
								<?php
									$distcode = $this-> session-> District; 
									$query="SELECT distinct facode, facilityname(facode) as facility from red_strategy_db where distcode='{$distcode}' order by facility ASC";
									$result = $this->db->query($query)->result_array();
								?>
							...
							<div class="col-md-3">
								<?php
									$distcode = $this-> session-> District; 
									$query="SELECT distinct year from red_strategy_db where distcode='{$distcode}' order by year DESC";
									$result = $this->db->query($query)->result_array();
								?>
						...
						<thead>
							<tr>
								...
								<th class="text-center Heading">
									<a href="<?php echo base_url(); ?>red_microplan/Red_strategy/red_strategy_add" data-toggle="tooltip" title="Add New Supervisor">
							...
							?>
								<tr>
							      ...
							      <td class="text-center">
								      <a href="<?php echo base_url(); ?>red_microplan/Red_strategy/red_strategy_view/<?php echo $row['facode']; ?>/<?php echo $row['year']; ?>" data-toggle="tooltip" title="View" class="btn btn-xs btn-default"><i class="fa fa-search"></i></a>
								      <a href="<?php echo base_url(); ?>red_microplan/Red_strategy/red_strategy_edit/<?php echo $row['facode']; ?>/<?php echo $row['year']; ?>" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
							      </td>
						 		</tr>
							<?php } ?>
						</tbody>
	...
	<script type="text/javascript">
		...
		$('.filter-status').on('change' , function (){
			...
			$.ajax({
				...
				dataType: "json",					
				url: "<?php echo base_url(); ?>Ajax_red_rec/red_strategy_filter",
