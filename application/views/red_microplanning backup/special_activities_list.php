	<div class="container">
		<div class="row">
			<div class="panel panel-primary">
				...
				<div class="panel-heading" style="font-size:17px; border-color:white !important;">Planning Special Activities List</div>
					...
					<form method="post" id="filter-form">
						<div class="row" style="width:100%; padding:4px 17px">
							...
							<div class="col-md-3">
								<?php
									...
									$query="SELECT distinct tcode, tehsilname(tcode) as tehsil from special_activities_db where distcode='{$distcode}' order by tehsil ASC";
							...
							<div class="col-md-3">
								<?php
									$distcode = $this-> session-> District; 
									$query="SELECT distinct facode, facilityname(facode) as facility from special_activities_db where distcode='{$distcode}' order by facility ASC";
									$result = $this->db->query($query)->result_array();
								?>
						...
						<div class="row" style="width:100%; padding:4px 17px">					
							...
							<div class="col-md-3">
								<?php
									$distcode = $this-> session-> District; 
									$query="SELECT distinct area_name from special_activities_db where distcode='{$distcode}' order by area_name ASC";
									$result = $this-> db-> query($query)->result_array();
								?>
							...
							<div class="col-md-3">
								<?php
									$distcode = $this-> session-> District; 
									$query="SELECT distinct year from special_activities_db where distcode='{$distcode}' order by year DESC";
									$result = $this->db->query($query)->result_array();
								?>
					...
					<table class="table footable table-bordered table-hover table-sessiontype" data-filter="#filter" data-filter-text-only="true">
						<thead>
							<tr>
								...
								<th>Category</th>
								<th>Hard to Reach</th>
								<th>Times Reached Last Year</th>
								<th>Year</th>
								<th>Tehsil</th>
								<th>Facility Name</th>
								<th class="text-center Heading">
									<a href="<?php echo base_url(); ?>red_microplan/Special_activities/special_activities_add" data-toggle="tooltip" title="Add New Supervisor">
						...
						<tbody id="tbody"> 
							...
							?>
								<tr>
							      ...
							      <td class="text-center"><?php echo $row['reached_last_year']; ?></td>
							      <td class="text-center"><?php echo $row['year']; ?></td>
							      <td><?php echo $row['tehsil']; ?></td>
							      <td><?php echo $row['facility']; ?></td>
							      <td class="text-center">
								      <a href="<?php echo base_url(); ?>red_microplan/Special_activities/special_activities_view/<?php echo $row['facode']; ?>/<?php echo $row['year']; ?>" data-toggle="tooltip" title="View" class="btn btn-xs btn-default"><i class="fa fa-search"></i></a>
								      <a href="<?php echo base_url(); ?>red_microplan/Special_activities/special_activities_edit/<?php echo $row['facode']; ?>/<?php echo $row['year']; ?>" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
							      </td>
						 		</tr>
							<?php } ?>
						</tbody>
	...
	<script type="text/javascript">
		...
		$('.filter-status').on('change' , function (){
			...
			$.ajax({
				...
				dataType: "json",					
				url: "<?php echo base_url(); ?>Ajax_red_rec/special_activities_filter",
