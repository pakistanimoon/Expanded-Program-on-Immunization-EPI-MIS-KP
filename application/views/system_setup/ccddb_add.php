...
<div class="row">
    <div class="panel panel-primary">
     <ol class="breadcrumb">
      ...
      <div class="panel-heading">  New Cold Chain Driver Form
        </div>
         <div class="panel-body">
 	 <form name="dataform" id="dataform" action="<?php echo base_url(); ?>System_setup/ccddb_save" method="post" enctype="multipart/form-data" class="form-horizontal form-bordered" onSubmit="">			
							...
							<label class="col-xs-2 control-label"  for = "tcode" > Tehsil </label>
								<div class="col-xs-3">
								 <select id="tcode" name="tcode" class="form-control" size="1" > <option value="">Select Tehsil</option>
									...
									<div class="row">
									<label class="col-xs-2 control-label"  for = "facode" > EPI Center Name </label>
								<div class="col-xs-3">
									...
									<input type="hidden" name="ccdcode" id="ccdcode" required value="<?php echo set_value('ccdcode'); ?>" >
									<div class="col-xs-1 cmargin18">
									<input type="text" disabled="disabled"  class="form-control  right" style="text-align: -webkit-right;" id="ccdcodef" value="CCD.Code" />
									</div>
									<div class="col-xs-2 cmargin19">
										<input type="text" style=" text-align: -webkit-left;" required name="ccdcodel" id="ccdcodel" placeholder="code"  class="form-control " value="<?php echo set_value('ccdcodel'); ?>"/>
									</div>
									</div>
									<div class="row">
									<label class="col-xs-2 control-label"  for = "ccdname" >Cold Chain Driver Name  </label>
									<div class="col-xs-3">
										<input type="text"  name="ccdname" id="ccdname" placeholder="Cold Chain Driver Name"  class="form-control " value="<?php if(validation_errors() != false) { echo set_value('ccdname'); }else{ } ?>"/><?php echo form_error('ccdname'); ?>
									</div>
...
$("#status").bind('change', function(){
    ...
    //etc ...
          ...
          $('#dataform').on('submit', function(e){
            ...
            if(checkCode(code)){
              ...
              alert('Please Enter Correct Cold Chain Driver Code');
              $('ccdcodel').focus();
            }
          });
          $('#nic').blur(function (){
          });
          $('#ccdcodel').keyup(function (){
            var ccdcodel = $('#ccdcodel').val();
            var ccdcodef = +$('#ccdcodef').val();
            var facode = "";
            if(/^\d+$/.test(ccdcodel) )
            {
              if($('#ccdcodef').val().length == 4 && $('#ccdcodel').val().length == 2 )
              {
                //alert('asdkjhsdja');
                $('#ccdcodel').css('border-color','#dbe1e8');
                 var distcode = '<?php echo $this -> session -> District; ?>';
                var ccdcodel  = $('#ccdcodel').val();
                var newVal = distcode+""+ccdcodel;
                //alert(newVal);
                $('#ccdcode').val(newVal);
                //alert($('#lhscode').val());
                $.ajax({
                  type: "GET",
                  data: "chkccdcode="+newVal,
 ...
 $(document).ready(function(){
	 var procode='3';
   var distcode = '<?php echo $this -> session -> District; ?>';
  $('#ccdcodef').val(procode+""+distcode); 
  
  var ccdcodel  = $('#ccdcodel').val();
  $.ajax({
    type: "GET",
    data: "ccdcode="+distcode,
    url: "<?php echo base_url(); ?>Ajax_calls/checkCodes",
    success: function(result){
      $('#ccdcodel').val(result);
      ccdcodel  = $('#ccdcodel').val();
      var newVal = procode+""+distcode+""+ccdcodel;
      $('#ccdcode').val(newVal);
    }
  });
});
...
$(document).ready(function() {    
    $('#ccdname, #fathername, #permanent_address, #present_address, #branchname').on('keyup', function(event) {
        var $this = $(this),
            val = $this.val();
