...
<div class="row">
  <?php if($this -> session -> flashdata('message')){  ?><div class="alert alert-success text-center" role="alert"><strong><?php echo $this -> session -> flashdata('message'); ?></strong></div> <?php } ?>
   ...
   <ol class="breadcrumb">
      ...
      <div class="panel-heading"> List of Cold Chain Mechanic
        </div>
...
<?php }?>
      ...
      <div class="row cmargin28">                
         ...
         <label class="col-xs-2 control-label lbl-setting"  for = "uncode" >UC:</label>
           <div class="col-xs-2">
...
<table  class="table table-bordered table-hover table-striped footable table-vcenter tbl-listing" data-filter="#filter" data-filter-text-only="true">
            <thead>
              <tr>
                ...
                <th class="text-center Heading">Mechanic Code</th>                
...
//echo '<pre>';print_r($results);exit(); 
    ...
    <tr class="DrilledDown"> 
      ...
      <td class="text-left"><?php echo $row['ccmname'];?></td>
      <td class="text-center"><?php echo $row['ccmcode'];?></td>      
      ...
      <td class="text-center">
        <a data-original-title="View" href="<?php echo base_url(); ?>CCM/View/<?php echo $row['ccmcode']?>" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-search"></i></a>
        <a data-original-title="Edit" href="<?php echo base_url(); ?>CCM/Edit/<?php echo $row['ccmcode']?>" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
      ...
      $('#tbody').html('<h1><td colspan="10" class="text-center" ><img src="<?php echo base_url(); ?>includes/images/ajax-loader_blue.gif"> loading...</td></h1>');
        ...
        $.ajax({
         ...
         // data: $('#filter-form').serialize(),
          dataType:"json",
          url: "<?php echo base_url(); ?>Ajax_calls/ccm_filter/"+page+"/"+distcode+"/"+tcode+"/"+uncode+"/"+facode+"/"+fatype,
      ...
      if($("#cnicSearch").val()!="")
      {
       var cnic= document.getElementById("cnicSearch").value;
            ...
            $.ajax({
              type: "GET",
              //data: $('#filter-form').serialize(),
              url: "<?php echo base_url(); ?>Ajax_calls/ccm_filter_cnic/"+cnic,
              dataType: "json",
              success: function(result){
                //console.log(result);
                $('#tbody').html('');
...
$('.DrilledDown').css('cursor','pointer');
    $(document).on('click',".DrilledDown", function(){
       var ccmcode = $(this).find("td:nth-child(2)").text();
        var url = '';
        url = "<?php echo base_url();?>CCM/View/"+ccmcode;
