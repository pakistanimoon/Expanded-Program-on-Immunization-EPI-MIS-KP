 	 ...
 	 <form name="dataform" id="dataform" action="<?php echo base_url(); ?>System_setup/cctdb_save" method="post" enctype="multipart/form-data" class="form-horizontal form-bordered" onSubmit="">			
							...
							<label class="col-xs-2 control-label"  for = "tcode" > Tehsil </label>
								...
								<div class="col-xs-3">
									...
									<label class="col-xs-2 control-label"   for = "cctcode" > Cold Chain Technician Code </label>
									<input type="hidden" name="cctcode" id="cctcode" required value="<?php echo set_value('cctcode'); ?>" >
									<div class="col-xs-1 cmargin18">
									<input type="text" disabled="disabled"  class="form-control  right" style="text-align: -webkit-right;" id="cctcodef" value="CCT.Code" />
									</div>
									<div class="col-xs-2 cmargin19">
										<input type="text" style=" text-align: -webkit-left;" required name="cctcodel" id="cctcodel" placeholder="code"  class="form-control " value="<?php echo set_value('cctcodel'); ?>"/>
									</div>
									</div>
									<div class="row">
									<label class="col-xs-2 col-xs-offset-1 control-label"  for = "cctname" >Cold Chain Technician Name  </label>
									<div class="col-xs-3">
										<input type="text"  name="cctname" id="cctname" placeholder="Cold Chain Technician Name"  class="form-control " value="<?php if(validation_errors() != false) { echo set_value('cctname'); }else{ } ?>"/><?php echo form_error('cctname'); ?>
									</div>
...
$("#status").bind('change', function(){
    ...
    //etc ...
          ...
          $('#dataform').on('submit', function(e){
            ...
            if(checkCode(code)){
              ...
              alert('Please Enter Correct Cold Chain Technician Code');
              $('#cctcodel').focus();
            }
          });
          $('#nic').blur(function (){
          });
          $('#cctcodel').keyup(function (){
            var cctcodel = $('#cctcodel').val();
            var cctcodef = +$('#cctcodef').val();
            var facode = "";
            if(/^\d+$/.test(cctcodel) )
            {
              if($('#cctcodef').val().length == 4 && $('#dsocodel').val().length == 3 )
              {
                //alert('asdkjhsdja');
                $('#cctcodel').css('border-color','#dbe1e8');
                 var distcode = '<?php echo $this -> session -> District; ?>';
                var cctcodel  = $('#cctcodel').val();
                var newVal = distcode+""+cctcodel;
                //alert(newVal);
                $('#cctcode').val(newVal);
                //alert($('#lhscode').val());
                $.ajax({
                  type: "GET",
                  data: "chkcctcode="+newVal,
 ...
 $(document).ready(function(){
	 var procode='3';
   var distcode = '<?php echo $this -> session -> District; ?>';
  $('#cctcodef').val(procode+""+distcode); 
  
  var cctcodel  = $('#cctcodel').val();
  $.ajax({
    type: "GET",
    data: "cctcode="+distcode,
    url: "<?php echo base_url(); ?>Ajax_calls/checkCodes",
    success: function(result){
      $('#cctcodel').val(result);
      cctcodel  = $('#cctcodel').val();
      var newVal = procode+""+distcode+""+cctcodel;
      $('#cctcode').val(newVal);
    }
  });
});
...
$(document).ready(function() {    
    $('#cctname, #fathername, #permanent_address, #present_address, #branchname').on('keypress', function(event) {
        var $this = $(this),
            val = $this.val();
