...
<div class="row">
  <?php if($this -> session -> flashdata('message')){  ?><div class="alert alert-success text-center" role="alert"><strong><?php echo $this -> session -> flashdata('message'); ?></strong></div> <?php } ?>
   ...
   <ol class="breadcrumb">
      ...
      <div class="panel-heading"> List of Cold Chain Technician
        </div>
              ...
              <tr>
                ...
                <?php if ($_SESSION['UserLevel']=='3' && $utype=='DEO' ){?>
                  <th class="text-center Heading">
                     <a href="<?php echo base_url(); ?>CCT/Add" data-toggle="tooltip" title="Add New Cold Chain Technician">
...
//echo '<pre>';print_r($results);exit(); 
    ...
    <tr class="DrilledDown"> 
      ...
      <td class="text-left"><?php echo $row['cctname'];?></td>
      <td class="text-center"><?php echo $row['cctcode'];?></td>      
      ...
      <td class="text-center">
        <a data-original-title="View" href="<?php echo base_url(); ?>CCT/View/<?php echo $row['cctcode']?>" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-search"></i></a>
        <a data-original-title="Edit" href="<?php echo base_url(); ?>CCT/Edit/<?php echo $row['cctcode']?>" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
      ...
      $('#tbody').html('<h1><td colspan="10" class="text-center" ><img src="<?php echo base_url(); ?>includes/images/ajax-loader_blue.gif"> loading...</td></h1>');
        ...
        $.ajax({
         ...
         // data: $('#filter-form').serialize(),
          dataType:"json",
          url: "<?php echo base_url(); ?>Ajax_calls/cct_filter/"+page+"/"+distcode+"/"+tcode+"/"+uncode+"/"+facode+"/"+fatype,
 ...
 $('#cnicbtn').on('click' , function (e){
      ...
      {  
            ...
            $.ajax({
              type: "GET",
              //data: $('#filter-form').serialize(),
              url: "<?php echo base_url(); ?>Ajax_calls/cct_filter_cnic/"+cnic,
              dataType: "json",
              success: function(result){
                //console.log(result);
                $('#tbody').html('');
...
$('.DrilledDown').css('cursor','pointer');
    $(document).on('click',".DrilledDown", function(){
       var cctcode = $(this).find("td:nth-child(2)").text();
        var url = '';
        url = "<?php echo base_url();?>CCT/View/"+cctcode;
