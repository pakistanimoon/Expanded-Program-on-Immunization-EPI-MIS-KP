...
<div class="row">
    <div class="panel panel-primary">
     <ol class="breadcrumb">
      ...
      <div class="panel-heading">  New Cold Chain Mechanic Form
        </div>
         <div class="panel-body">
 	 <form name="dataform" id="dataform" action="<?php echo base_url(); ?>System_setup/ccmdb_save" method="post" enctype="multipart/form-data" class="form-horizontal form-bordered" onSubmit="">			
							...
							<label class="col-xs-2 control-label"  for = "tcode" > Tehsil </label>
								<div class="col-xs-3">
								 <select id="tcode" name="tcode" class="form-control" size="1" > <option value="">Select Tehsil</option>
									...
									<label class="col-xs-2 control-label"   for = "ccmcode" > Cold Chain Mechanic Code </label>
									<input type="hidden" name="ccmcode" id="ccmcode" required value="<?php echo set_value('ccmcode'); ?>" >
									<div class="col-xs-1 cmargin18">
									<input type="text" disabled="disabled"  class="form-control  right" style="text-align: -webkit-right;" id="ccmcodef" value="CCM.Code" />
									</div>
									<div class="col-xs-2 cmargin19">
										<input type="text" style=" text-align: -webkit-left;" required name="ccmcodel" id="ccmcodel" placeholder="code"  class="form-control " value="<?php echo set_value('ccmcodel'); ?>"/>
									</div>
									</div>
									<div class="row">
									<label class="col-xs-2 col-xs-offset-1 control-label"  for = "ccmname" >Cold Chain Mechanic Name  </label>
									<div class="col-xs-3">
										<input type="text"  name="ccmname" id="ccmname" placeholder="Cold Chain Mechanic Name"  class="form-control " value="<?php if(validation_errors() != false) { echo set_value('ccmname'); }else{ } ?>"/><?php echo form_error('ccmname'); ?>
									</div>
...
$("#status").bind('change', function(){
    ...
    //etc ...
          ...
          $('#dataform').on('submit', function(e){
            ...
            if(checkCode(code)){
              ...
              alert('Please Enter Correct Cold Chain Mechanic Code');
              $('ccmcodel').focus();
            }
          });
          $('#nic').blur(function (){
          });
          $('#ccmcodel').keyup(function (){
            var ccmcodel = $('#ccmcodel').val();
            var ccmcodef = +$('#ccmcodef').val();
            var facode = "";
            if(/^\d+$/.test(ccmcodel) )
            {
              if($('#ccmcodef').val().length == 4 && $('#ccmcodel').val().length == 2 )
              {
                //alert('asdkjhsdja');
                $('#ccmcodel').css('border-color','#dbe1e8');
                 var distcode = '<?php echo $this -> session -> District; ?>';
                var ccmcodel  = $('#ccmcodel').val();
                var newVal = distcode+""+ccmcodel;
                //alert(newVal);
                $('#ccmcode').val(newVal);
                //alert($('#lhscode').val());
                $.ajax({
                  type: "GET",
                  data: "chkccmcode="+newVal,
 ...
 $(document).ready(function(){
	 var procode='3';
   var distcode = '<?php echo $this -> session -> District; ?>';
  $('#ccmcodef').val(procode+""+distcode); 
  
  var ccmcodel  = $('#ccmcodel').val();
  $.ajax({
    type: "GET",
    data: "ccmcode="+distcode,
    url: "<?php echo base_url(); ?>Ajax_calls/checkCodes",
    success: function(result){
      $('#ccmcodel').val(result);
      ccmcodel  = $('#ccmcodel').val();
      var newVal = procode+""+distcode+""+ccmcodel;
      $('#ccmcode').val(newVal);
    }
  });
});
...
$(document).ready(function() {    
    $('#ccmname, #fathername, #permanent_address, #present_address, #branchname').on('keypress', function(event) {
        var $this = $(this),
            val = $this.val();
