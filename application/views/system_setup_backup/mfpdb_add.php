...
<div class="container bodycontainer">
  ...
  <div class="row">
    <div class="panel panel-primary">
     ...
     <div class="panel-heading"> New Measles Focal Person Form
     </div>
 ...
 <!-- <div class="panel-body">
     ...
     <!--<label class="col-xs-2 control-label"   for = "cocode" > Computer Operator Code </label>-->
              <input type="hidden" name="mfpcode" id="mfpcode" required value="<?php echo set_value('mfpcode'); ?>" >
              <div class="col-xs-1 cmargin18">
                <input type="hidden" disabled="disabled"  class="form-control  right" style="text-align: -webkit-right;" id="mfpcodef" value="MFP.Code" />
              </div>
                <div class="col-xs-2 cmargin19">
               <input type="hidden" style=" text-align: -webkit-left;" required name="mfpcodel" id="mfpcodel" placeholder="code"  class="form-control " value="<?php echo set_value('mfpcodel'); ?>"/>
               </div>
            ...
            <div class="row">
               ...
               <label class="col-xs-2 col-xs-offset-1 control-label"  for = "mfpname" > Measles Focal Person Name <span style="color:red;">*</span></label>
               <div class="col-xs-3">
                 <input type="text" required name="mfpname" id="mfpname" placeholder="Measles Focal Person Name"  class="form-control " value="<?php echo set_value('mfpname'); ?>" /> <?php echo form_error('mfpname'); ?>   </div>
             ...
             <div class="row bgrow" style="text-align: center;color: white;font-size: 15px; font-family: Arial;padding-top: 5px; "> Basic Information</div>
              ...
              <div class="row">
               ...
               <label class="col-xs-2 col-xs-offset-1 control-label"  for = "date_of_birth" >Date of Birth <span style="color:red;">*</span></label>
                <div class="col-xs-3">
               ...
               <label class="col-xs-2 control-label"  for = "nic" > CNIC # <span style="color:red;">*</span></label>
                  <div class="col-xs-3">
                     <input  name="nic" required id="nic" placeholder="Enter Your CNIC #"  class="form-control " value="<?php echo set_value('nic'); ?>"/><span id="site_response"></span>
                  </div>
...
//$('#distcode').on('change' , function (){
  ...
  $('#mfpcodel').val('');
  $('#mfpcodef').val(distcode);
  
  $.ajax({
    type: "GET",
    data: "distcodemfp="+distcode,
    async: false,
    url: "<?php echo base_url(); ?>Ajax_calls/checkCodes",
    success: function(result){
      var resCode=result.trim();
      $('#mfpcodel').val(resCode);
      var mfpcodel  = $('#mfpcodel').val();
      var newVal = distcode+""+mfpcodel;
      $('#mfpcode').val(newVal);
		//alert(newVal);
    }
...
<script type="text/javascript">
  ...
  $(document).on('blur','#nic', function(){
    ...
    $.ajax({ 
      type: 'POST',
      data: "nic="+nic,
      url: '<?php echo base_url();?>Ajax_calls/checkMfpNIC',
    ...
    success: function(data){
     if(data!=''){
      if(data=='yes'){
        ...
        $("#site_response").html('CNIC Already Exist For Another Measles Focal Person.');
	...
	$(document).ready(function() {    
    $('#mfpname, #fathername, #permanent_address, #present_address, #branchname').on('keyup', function(event) {
        var $this = $(this),
            val = $this.val();
