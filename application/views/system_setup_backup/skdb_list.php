...
<?php if($this -> session -> flashdata('message')){  ?><div class="alert alert-success text-center" role="alert"><strong><?php echo $this -> session -> flashdata('message'); ?></strong></div> <?php } ?>
    ...
    <div class="panel panel-primary">
      ...
      <div class="panel-heading"> List of Store Keeper
...
<table id="skdb-tbl" class="table table-bordered table-hover table-striped footable table-vcenter tbl-listing" data-filter="#filter" data-filter-text-only="true" style="text-align: center;">
  ...
  <?php if ($_SESSION['UserLevel']=='3' && $utype=='DEO' ){?> 
                <th class="text-center Heading">               
				  <!-- <a href="<?php echo base_url(); ?>Store-keeper/Add" data-toggle="tooltip" title="Add Store keeper">                 
				   <button id="B_add" class="submit btn-success btn-sm">
                    <i class="fa fa-plus"></i> Add New</button>
                  </a> --> 
                  <a href="<?php echo base_url();?>Store-keeper/Add"  class="btn-primary submit"  style="border:none; border-radius: 2px;border: none;
                  border-radius: 2px; background: #337ab7 !important; padding: 3px 8px;">
  ...
  <?php
      ...
      foreach($results as $row){
        ...
        <tr class="DrilledDown">
          ...
          <td class="text-left" ><?php echo $row['skname']; ?></td>
        ...
        <?php if ($_SESSION['UserLevel']=='3'  || $_SESSION['utype']=='DEO'){?>
          ...
          <td class="text-center">
                      ...
                      <a data-original-title="View" href="<?php echo base_url(); ?>Store-keeper/View/<?php echo $row['skcode']; ?>" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-search"></i></a>

                      <a data-original-title="Edit" href="<?php echo base_url(); ?>Store-keeper/Edit/<?php echo $row['skcode']; ?>" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
...
<script type="text/javascript">
    ...
    var columns = [
          ...
          { data: "skcode" ,
            ...
            render : function(data, type, row) {
                return '<a data-original-title="View" href="<?php echo base_url(); ?>system_setup/skdb_view/'+data+'" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-search"></i></a><a data-original-title="Edit" href="<?php echo base_url(); ?>system_setup/skdb_edit/'+data+'" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>'
            }  
          }
        ]; 
  <?php } elseif ($_SESSION['utype']=='Manager'){?>
    var columns = [
          ...
          { data: "skcode" ,
            orderable: false,
            render : function(data, type, row) {
                return '<a data-original-title="View" href="<?php echo base_url(); ?>system_setup/skdb_view/'+data+'" data-toggle="tooltip" title="" class="btn btn-xs btn-default"><i class="fa fa-search"></i></a>'
            }  
          }
        ]; 
  <?php } ?>
  var table = $('#skdb-tbl').DataTable({
        "pageLength" : 16,
        "serverSide": true,
        "order": [
          [1, "desc" ]
        ...
        "ajax": {
            /*url : "<?php echo base_url(); ?>Ajax_calls/sk_dataTables",*/
             url : "<?php echo base_url(); ?>Ajax_calls/sk_dataTables",
            type : 'GET'
        },
        "columns": columns,
 ...
 /* $(document).on('click','#B_add',function(){
		window.open("<?php echo base_url(); ?>Store-keeper/Add");
	});*/
