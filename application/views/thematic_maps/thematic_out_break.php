				...
				<div class="carview_wraper">
					<ul class="row">
						<?php
							...
							*/
							$cardOne['heading'] 	= ((isset($id) && $id !="")?"Uc":"District")." With OutBreak";
							$cardOne['image'] 		= "chart1.png";
							$cardOne['value'] 		= number_format($withdrop,0); // For Numeric Values Send value formated in number_format.
							$cardOne['width'] 		= 6;
							$cardTwo['heading'] 	= "Total No of OutBreak";
							$cardTwo['image'] 		= "peichart.jpg";
							$cardTwo['value'] 		= number_format($totaldrops,0); // For Numeric Values Send value formated in number_format.
							$cardTwo['width'] 		= 6;
		...
		<div class="filter_formwrp">
			...
			<?php 
				...
				*/
				...
				$filterRow['disease']= $disease;
				$filterRow['from_week']= $from_week;
				$filterRow['to_week']= $to_week;
				$filters['filter'] = 'outbreak';
...
<script type="text/javascript">
	...
	function formatter(e,ucwisemap='false'){
		...
		return text+': <b>' + e.point.name + ' (' + e.point.id + ')' + '</b><br> Year-Week: <b>' + '<?php echo $fweekhover; ?>' + '</b><br> ' + 'No Disease OutBreak: <b>' + e.point.y + '</b>';
	}
	
	function eventHandler(e, run, fmonth){
		var mobiletype = '<?php echo $this->agent->is_mobile(); ?> ';
		var dataId = ""+e.point.id;
		var reportType = 'Weakly';
		...
		var casetype='cases'
		 if(run){
			var url = '<?php echo base_url(); ?>thematic_maps/ThematicOutbreak/UcWiseMapData/'+dataId+'/'+year+'/'+from_week+'/'+to_week+'/'+disease;
			...
			if(dataId > 0 && mobiletype !=true){
				$.ajax({
					...
					data: data,
					async:false,
					url: "<?php echo base_url(); ?>thematic_maps/ThematicMorbidity/getUC_detailData",
	...
	/* function getDistrictWiseData(e, fmonth){
		...
		if(dataId > 0){
			$.ajax({
				...
				data: data,
				async:false,
				url: "<?php echo base_url(); ?>thematic_maps/ThematicOutbreak/UcWiseMapData",
				success: function(result){
					$('.ucdetailsdata').html(result);
					$('.ucdetailsdata').removeClass('hide');
					$('.filterRow').removeClass('hide');
