...
alasql("CREATE TABLE modalData (prodid string, antigen string, htmldata string, totalrec number)");
//var serializedString = "";
//var $form = $('#modalForm-dataSharing');
$(document).ready(function(){
	...
	$(document).on('click','.sharedata',function(){
		...
		if(oldprodname==name && oldantigen==antigen){
			...
			var res = alasql("SELECT * FROM modalData WHERE prodid = '"+prodid+"' and antigen = '"+antigen+"'");
			if(res === undefined || res.length == 0){
				$(modalobj).find("#sharingdatatblebody").html($.parseHTML(defaultval[0].htmldata));
			}else{
				//data from lcoal db
				for(var iter = 0; iter<res[0].totalrec;iter++){
					if(iter==0){
						$(modalobj).find("#sharingdatatblebody").html($.parseHTML(defaultval[0].htmldata));
					}else{
				...
				$(modalobj).find("#modalForm-dataSharing").deserialize(res[0].htmldata);
			}
		}
	});	
...
$(document).on('click','#submitDataSharing',function(){
	...
	var serializedString = $(modalobj).find("#modalForm-dataSharing").serialize();
	$.ajax({
		type: "post",
		data: serializedString+"&year="+year+"&month="+month+"&facode="+facode+"&item_id="+item_id+"&antigen="+antigen,
