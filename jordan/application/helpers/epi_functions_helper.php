...
if(!function_exists('GovernorateName')){
    function GovernorateName($govcode="")
    {  
        $CI=& get_instance();
        $_query  = "SELECT govname from jor_governorate where govcode = '$govcode'";
...
if(!function_exists('getEpiWeekYearsOptions')){
	function getEpiWeekYearsOptions($epiYear=NULL,$isreturn=false){
		$CI = & get_instance();
...
if(!function_exists('getAllMonthsOptions')){
	function getAllMonthsOptions($isreturn=false){		
		...
		$mnth = isset($_REQUEST["month"])?$_REQUEST["month"]:date('m');//date('m',strtotime("first day of previous months")
		$currMon = ($mnth=="13")?12:date('m');//date('m',strtotime("first day of previous months")
...
if(!function_exists('getVaccines_options')){
	function getVaccines_options($isreturn=false,$activity=1,$returndata = FALSE,$catin=array()){
		$CI = & get_instance();
		$output = "";
		...
		foreach ($result1 as $vaccines) { 
			...
			$output .= '<option value="'.$vaccines["itemid"].'" >'.$vaccname.'</option>';
		}
		if($isreturn)
			return $output;
		echo $output;
...
if(!function_exists('getSourceSupply')){
	function getSourceSupply($id=null,$text=FALSE){
		$return='
				...
				<option valus="7" '.(($id==7)?'selected="selected"':'').'>JICA</option>
