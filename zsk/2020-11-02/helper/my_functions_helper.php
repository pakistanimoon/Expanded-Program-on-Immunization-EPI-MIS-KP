	...
	function getListingReportTable($allData,$listingFor,$allTotal=NULL,$sectionTotal = 'Yes'){
		if(!empty($allData)){
			...
			foreach($allData as $key => $value)
			{
				...
				$returnData .= implode("</td><td style='text-align:center; border: 1px solid black;' ", array_map(function($v)/* use(&$class) */
				{
					if(is_numeric($v)){
...
if(!function_exists('getFLCF_optionsbyuncode')){
	function getFLCF_optionsbyuncode($isreturn=false,$facode=NULL,$uncode=NULL){
		...
		$query="SELECT fac_name, facode from facilities where uncode='$uncode' and hf_type='e' order by fac_name";
...
if(!function_exists('getSectionsStatusTableHR')){
	function getSectionsStatusTableHR($hrstatus,$hrstatus_counts){
		$CI = & get_instance();
        ...
        $i=0;
		$count_status=$hrstatus_counts[0]['count'];
		foreach($hrstatus as $key => $value)
        {
			...
			$url=base_url(); 
			$body .='</td><td>'; 
			if(!($del && $i == 1)){}elseif(($UserLevel == 2 || $UserLevel == 3) && $utype == 'Manager' || $utype == 'DEO' && $count_status > 1){ 
