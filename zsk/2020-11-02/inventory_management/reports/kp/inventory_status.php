...
<?php }?>
				<table class="table table-bordered table-condensed table-striped table-hover mytable"      border="1" >
					<thead>
						<tr>
							<th rowspan="2"><?php echo $colname; ?></th>
							<th colspan="3">Vouchers issued to <?php echo $store; ?></th>
							<th colspan="3">Vouchers issued from <?php echo $store; ?></th>
						</tr>
						<tr>
							<th>Issued</th>
							<th>Received</th>
							<th>Pending Receiving</th>
							<th>Issued</th>
							<th>Received</th>
							<th>Pending Receiving</th>
						</tr>
					...
					<tbody>
						...
						foreach($reportdata as $key=>$row){
							...
							*/?>
							<tr>
								<th><?php echo $firstcolval; ?></th>
								<td <?php echo ($row["issuedme"])?'class="drilldownCell" data-class="issuedme" data-month="'.$row["repbyname"].'"':''; ?>><?php echo $row["issuedme"]; ?></td>
								<td <?php echo ($row["receivedme"])?'class="drilldownCell" data-class="receivedme" data-month="'.$row["repbyname"].'"':''; ?>><?php echo $row["receivedme"]; ?></td>
								<?php $pending = $row["issuedme"]-$row["receivedme"]; ?>
								<td <?php echo (isset($pending) && $pending>0)?'class="drilldownCell" data-class="pendingme" data-month="'.$row["repbyname"].'"':''; ?>><?php echo $pending; ?></td>
								<td <?php echo ($row["issuedother"])?'class="drilldownCell" data-class="issuedother" data-month="'.$row["repbyname"].'"':''; ?>><?php echo $row["issuedother"]; ?></td>
								<td <?php echo ($row["receivedother"])?'class="drilldownCell" data-class="receivedother" data-month="'.$row["repbyname"].'"':''; ?>><?php echo $row["receivedother"]; ?></td>
								<?php $pendingother = $row["issuedother"]-$row["receivedother"]; ?>
								<td <?php echo (isset($pendingother) && $pendingother>0)?'class="drilldownCell" data-class="pendingother" data-month="'.$row["repbyname"].'"':''; ?>><?php echo $pendingother; ?></td>
							</tr><?php
						}} else{?>
